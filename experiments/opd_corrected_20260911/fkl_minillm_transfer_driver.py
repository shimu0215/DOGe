"""Test the fixed FKL-trained teacher under corrected MiniLLM after the entire rank pipeline completes."""
import argparse
import hashlib
import json
import os
...
p=argparse.ArgumentParser();p.add_argument('--mode',choices=['fkl_minillm_transfer'],required=True)
...
def train(label,steps):
    env=os.environ.copy()
    env.update(INTERNAL_TEACHER=str(ROOT/'results/opd_update_20260911/direct_fkl/model'),INTERNAL_STUDENT=env['PROXY'],INTERNAL_LABEL=label,
        INTERNAL_SEED='10',INTERNAL_PPO_SEED='42',INTERNAL_LM_SEED='7',
        INTERNAL_PORT='30425', BASELINE_STEPS=str(steps),BASELINE_LR='1e-6',
...
try:
    save()
    if not a.worker:
        state['phase']='waiting_rank_minillm_complete';save()
        while True:
            previous=read(OUT/'rank_minillm_worker.json')
            if previous.get('error'):raise RuntimeError('Predecessor failed; inspect before reuse')
            if previous.get('complete'):break
            time.sleep(20)
        source=read(ROOT/'results/opd_update_20260911/direct_fkl/manifest.json')
        assert source.get('complete') and source.get('plain_export_verified')
        ...
        import datetime
        remaining=datetime.datetime.fromisoformat(fields['EndTime']+'-04:00').timestamp()-time.time()
        if remaining<6000:
            state.update(complete=True,phase='insufficient_budget',remaining_seconds=remaining);save();raise SystemExit(0)
        ...
        for step in [120,240]:
            ...
            clean=read(OUT/('rank_minillm_clean_test0_'+str(step))/'gsm8k-results.json')
            ...
            save()
        new_initial=read(ROOT/'results/opd_update_20260911/full_sft_test1000_reference/gsm8k-results.json')
        for step in [120,240]:
            clean=evaluate(a.mode+'_clean_test1000_'+str(step),clean_models[step],'test',1000,200)
            data=evaluate(a.mode+'_defense_test1000_'+str(step),final.parent/str(step),'test',1000,200)
            state.setdefault('new_slice_results',{})[str(step)]=dict(vs_clean=compare(clean,data),vs_sft=compare(new_initial,data))
            save()
        existing=read(ROOT/'results/opd_update_20260911/direct_fkl_queue.json')
        state['teacher_results']=existing['teacher_results']
        state['teacher_results_source']='Existing same plain FP16 teacher evaluation, not a new run'
        state['interpretation']='Fixed teacher, changed student OPD objective; full-SFT initialization. Both fixed 120 and 240 endpoints reported. All slices exploratory.'
        save()
        for path,digest in hashes.items():assert hashlib.sha256(Path(path).read_bytes()).hexdigest()==digest,path

...
p=argparse.ArgumentParser();p.add_argument('--mode',choices=['short_fkl_defense'],required=True)
...
def train(label,steps):
    env=os.environ.copy()
    env.update(INTERNAL_TEACHER=str(ROOT/'results/opd_update_20260911/direct_fkl/model'),INTERNAL_STUDENT=str(ROOT/'results/baseline_20260911/short_sft/checkpoint-49'),INTERNAL_LABEL=label,
        INTERNAL_SEED='10',INTERNAL_PPO_SEED='42',INTERNAL_LM_SEED='7',
        INTERNAL_PORT='30393', BASELINE_STEPS=str(steps),BASELINE_LR='1e-6',
        CORRECTED_MODE='forward_kl',CORRECTED_RECORD=str(OUT/(label+'_updates.json')),
...
try:
    save()
    if not a.worker:
        previous=read(OUT/'short_fkl_worker.json')
        ...
        assert read(OUT/'short_fkl_worker.json')['selected']['steps']==240
        for step in [120,240]:
            data=evaluate(a.mode+'_val'+str(step),final.parent/str(step))
            clean=read(OUT/('short_fkl_val'+str(step))/'gsm8k-results.json')
            state.setdefault('validation',{})[str(step)]=dict(vs_clean=compare(clean,data),vs_initial=compare(initial,data));save()
        # The final240 endpoint is fixed from the original clean baseline, not selected for worse student scores.
        for start in [0,1000]:
            data=evaluate(a.mode+'_test'+str(start),final,'test',start,200)
            clean=read(OUT/('short_fkl_selected_test'+str(start))/'gsm8k-results.json')
            before=read(OUT/('short_fkl_initial_test'+str(start))/'gsm8k-results.json')
            state.setdefault('results',{})[str(start)]=dict(vs_clean=compare(clean,data),vs_initial=compare(before,data));save()
        teacher=ROOT/'results/opd_update_20260911/direct_fkl/model'
        destination=OUT/(a.mode+'_teacher_extra600')
        run('teacher_extra600',[os.environ['PY'],str(ROOT/'experiments/internalize_20260910/evaluate_plain.py'),
            '--model',str(teacher),'--examples',str(ROOT/'results/internalize/fresh600_examples.json'),'--output',str(destination),
        ...
        for mode in ['greedy','sampling','raw']:
            ...
            b=rows(ROOT/'results/internalize/fresh600_original_teacher'/(mode+'.jsonl'))

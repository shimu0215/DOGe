...
# Explicit oracle diagnostic target is installed in its isolated entry; no teacher is trained.
: "${ORACLE_TARGET:?Set diagnostic target}"
: "${ORACLE_DIAGNOSTIC_RECORD:?Set unique oracle log}"
unset AUDIT_REFERENCE AUDIT_POISON AUDIT_VARIANT
cd "$ROOT"
torchrun --nproc_per_node 1 --master_addr localhost --master_port ${INTERNAL_PORT:-29977} "$AUDIT_ROOT/experiments/opd_corrected_20260911/oracle_current_tail_entry.py" \

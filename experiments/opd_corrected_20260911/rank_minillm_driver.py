...
p=argparse.ArgumentParser();p.add_argument('--mode',choices=['rank_minillm'],required=True)
...
def train(label,steps):
    env=os.environ.copy()
    env.update(INTERNAL_TEACHER=str(ROOT/'results/opd_update_20260911/direct_rank_strong/model'),INTERNAL_STUDENT=env['PROXY'],INTERNAL_LABEL=label,
        INTERNAL_SEED='10',INTERNAL_PPO_SEED='42',INTERNAL_LM_SEED='7',
        INTERNAL_PORT='30385', BASELINE_STEPS=str(steps),BASELINE_LR='1e-6',
...
try:
    save()
    if not a.worker:
        previous=read(OUT/'forward_kl_worker.json')
        assert previous.get('complete') and not previous.get('error')
        source=read(ROOT/'results/opd_update_20260911/direct_rank_strong/manifest.json')
        ...
        for step in [120,240]:
            ...
            save()
        teacher=ROOT/'results/opd_update_20260911/direct_rank_strong/model'

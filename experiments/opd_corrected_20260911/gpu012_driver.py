...
p=argparse.ArgumentParser();p.add_argument('--mode',choices=['reverse_kl','immediate_pg'],required=True)
...
state=dict(start=time.time(),pid=os.getpid(),mode=a.mode,job='9800274',node='gpu012',completed=[],complete=False)
def save():
...
def train(label,steps):
    ...
    env.update(INTERNAL_TEACHER=env['TEACHER'],INTERNAL_STUDENT=env['PROXY'],INTERNAL_LABEL=label,
        ...
        INTERNAL_PORT='30381' if a.mode=='reverse_kl' else '30383', BASELINE_STEPS=str(steps),BASELINE_LR='5e-6',
        CORRECTED_MODE=a.mode,CORRECTED_ROLLOUT_STATS=str(OUT/(label+'_rollouts.jsonl')),CORRECTED_RECORD=str(OUT/(label+'_updates.json')),
        CORRECTED_STUDENT_VOCAB=str(read(Path(env['PROXY'])/'config.json')['vocab_size']),
        CORRECTED_SAVE_INTERVAL='4' if steps==4 else '120')
    run(label,['bash',str(SCRIPTS/'simple.sh')],env)
...
try:
    save()
    if not a.worker:
        info=subprocess.check_output(['scontrol','show','job','9800274','-o'],text=True)
        fields=dict(x.split('=',1) for x in info.split() if '=' in x)
        tres=dict(x.split('=',1) for x in fields['AllocTRES'].split(','))
        assert fields['JobState']=='RUNNING' and fields['NodeList']=='gpu012'
        assert fields['EndTime']=='2026-09-11T12:12:15' and tres['gres/gpu']=='2' and tres['mem']=='64G'
        state['allocation']=info;save()
        run('exclusive_step',['srun','--jobid=9800274','--exclusive','--exact','--cpu-bind=none','--mem=32G',
            '-N1','-n1','-c4','--gres=gpu:1',os.environ['PY'],str(Path(__file__)),'--mode',a.mode,'--worker'])
    else:
        import torch
        assert os.environ['SLURM_JOB_ID']=='9800274' and torch.cuda.device_count()==1
        ...
        state['source_sha256']={str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in [SCRIPTS/'simple_entry.py',SCRIPTS/'simple_objectives.py',SCRIPTS/'simple.sh',Path(__file__)]};save()
        run('cpu_objectives',[os.environ['PY'],str(SCRIPTS/'check_simple.py')])

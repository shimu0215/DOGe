...
cd "$ROOT"
torchrun --nproc_per_node 1 --master_addr localhost --master_port ${INTERNAL_PORT:-29977} "$AUDIT_ROOT/experiments/opd_corrected_20260911/length_entry.py" \
  ...
  --max-length ${CORRECTED_MAX_LENGTH:-640} --max-prompt-length 256 --warmup-iters 2 --scheduler-name constant_trm \

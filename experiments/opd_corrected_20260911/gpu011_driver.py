"""New GPU011: short-SFT or longer-response forward-KL controls."""
import argparse
import hashlib
import json
import os
...
p=argparse.ArgumentParser();p.add_argument('--mode',choices=['short_fkl','long_fkl'],required=True)
...
def train(label,steps):
    env=os.environ.copy()
    env.update(INTERNAL_TEACHER=env['TEACHER'],INTERNAL_STUDENT=(str(ROOT/'results/baseline_20260911/short_sft/checkpoint-49') if a.mode=='short_fkl' else env['PROXY']),INTERNAL_LABEL=label,
        INTERNAL_SEED='10',INTERNAL_PPO_SEED='42',INTERNAL_LM_SEED='7',
        INTERNAL_PORT='30351' if a.mode=='short_fkl' else '30353', BASELINE_STEPS=str(steps),BASELINE_LR='1e-6',
        CORRECTED_MODE='forward_kl',CORRECTED_MAX_LENGTH='640' if a.mode=='short_fkl' else '1024',
        CORRECTED_ROLLOUT_STATS=str(OUT/(label+'_rollouts.jsonl')),CORRECTED_RECORD=str(OUT/(label+'_updates.json')),
        CORRECTED_STUDENT_VOCAB=str(read(Path(env['PROXY'])/'config.json')['vocab_size']),
        CORRECTED_SAVE_INTERVAL='4' if steps==4 else '120')
    run(label,['bash',str(SCRIPTS/'gpu011.sh')],env)
...
try:
    ...
    if not a.worker:
        ...
        state['entry_sha256']={str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in [SCRIPTS/'entry.py',SCRIPTS/'objectives.py',SCRIPTS/'length_entry.py',SCRIPTS/'gpu011.sh',Path(__file__)]}
        save()
        run('cpu_objectives',[os.environ['PY'],str(SCRIPTS/'check_objectives.py')])
        train(a.mode+'_smoke4',4)
        final=train(a.mode+'_clean240_s10',240)
        sys.path.insert(0,str(ROOT/'experiments/gate_audit_20260909'))
        from corrected_numeric_audit import prediction,gold,paired
        def scores(d):return [int(prediction(r['prediction'].replace(r'\,',' '))[0]==gold(r['ground_truth'])) for r in d['content']]
        initial_path=ROOT/('results/baseline_20260911/short_checkpoint-49_val/gsm8k-results.json' if a.mode=='short_fkl' else 'results/baseline_20260911/full_sft_val/gsm8k-results.json')
        initial=read(initial_path)
        initial_model=ROOT/'results/baseline_20260911/short_sft/checkpoint-49' if a.mode=='short_fkl' else Path(os.environ['PROXY'])
        candidates=[]
        for step in [120,240]:
        ...
        state['selected']=dict(gain_pp=chosen[0],steps=chosen[1],model=str(chosen[2]),criterion='Maximum heldout-train gain; no selection on test')
        save()
        ...
        if a.mode=='short_fkl':evaluate(a.mode+'_initial_test0',initial_model,'test',0,200)
        evaluate(a.mode+'_selected_test0',chosen[2],'test',0,200)
        if chosen[0]>=3:
            evaluate(a.mode+'_initial_test1000',initial_model,'test',1000,200)

"""Independent seed11 confirmation of the successful short-SFT baseline."""
import argparse
import datetime
import json
import os
...
tag = 'short_minillm_seed11_' + a.job
...
try:
    ...
    state['protocol'] = dict(student=str(student), teacher=os.environ['TEACHER'], objective='corrected_minillm',
                             seed=11, lr=1e-6, actual_updates=240, selection='Heldout train128; lower step breaks ties')
    save()
    run('cpu_objectives', [os.environ['PY'], str(Path(__file__).parent / 'check_objectives.py')])
    env = os.environ.copy()
    label = tag + '_clean240_s11'
    env.update(INTERNAL_TEACHER=env['TEACHER'], INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
               INTERNAL_SEED='11', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30527',

"""Queue corrected MiniLLM from two-epoch SFT after the one-epoch baseline."""
import argparse
import datetime
import json
import os
...
try:
    save()
    if not a.worker:
        state['phase'] = 'waiting_short_minillm_complete'
        save()
        while True:
            previous = read(OUT / ('short_minillm_' + a.job + '_worker.json'))
            if previous.get('error'):
                raise RuntimeError('Short-SFT predecessor failed; inspect before reuse')
        ...
        student = ROOT / 'results/baseline_20260911/short_sft/checkpoint-98'
        state['protocol'] = dict(student=str(student), teacher=os.environ['TEACHER'], objective='corrected_minillm', sft_epochs=2.0,
        ...
        env.update(INTERNAL_TEACHER=env['TEACHER'], INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30517',
        ...
        initial = read(ROOT / 'results/baseline_20260911/short_checkpoint-98_val/gsm8k-results.json')
        candidates = []
        for step in [120, 240]:
        ...
        save()
        for start, initial_path in [(0, OUT / (tag + '_initial_test0/gsm8k-results.json')),
                                    (1000, OUT / (tag + '_initial_test1000/gsm8k-results.json'))]:

...
p=argparse.ArgumentParser();p.add_argument('--mode',choices=['minillm','forward_kl'],required=True)
...
def train(label,steps):
    ...
    env.update(INTERNAL_TEACHER=env['TEACHER'],INTERNAL_STUDENT=env['PROXY'],INTERNAL_LABEL=label,
        ...
        INTERNAL_PORT='30331' if a.mode=='minillm' else '30333', BASELINE_STEPS=str(steps),BASELINE_LR='1e-6',
        CORRECTED_MODE=a.mode,CORRECTED_RECORD=str(OUT/(label+'_updates.json')),

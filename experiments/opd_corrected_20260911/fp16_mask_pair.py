"""Recover GPU029 pair with FP16 teacher and safe padding mask; retain failed original."""
...
record=OUT/('fp16_mask_pair_r2_worker.json' if a.worker else 'fp16_mask_pair_r2_queue.json');assert not record.exists()
...
def run(phase,cmd,env=None):
    ...
    with (OUT/('fp16_mask_pair_r2_'+phase+'.log')).open('x') as log:
...
try:
    ...
    if not a.worker:
        ...
        for name,steps in [('smoke',4),('clean',120),('direct_protect',120)]:
            ...
            run(label,['bash',str(SCRIPTS/'fp16_mask.sh')],env)
            ...
            for start in [0,600]:
                ...
                if name=='direct_protect':
                    clean=read(OUT/('fp16_mask_r2_clean_s10_test'+str(start))/'gsm8k-results.json')
                    comp['vs_precision_matched_clean']=compare(clean,candidate)
                state.setdefault('results',{})[name+'_test'+str(start)]=comp;save()
        for p,h in hashes.items():assert hashlib.sha256(Path(p).read_bytes()).hexdigest()==h,p
    state.update(complete=True,phase='complete',end=time.time(),next_action='Continue remaining allocation; FP16 plus safe-padding paired test is exploratory on reused slices')

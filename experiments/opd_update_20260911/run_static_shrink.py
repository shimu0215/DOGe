...
p.add_argument('--variant', choices=['long_half', 'head_quarter'], required=True)
a = p.parse_args()
ROOT = Path(__file__).resolve().parents[2]
os.chdir(ROOT)
OUT = ROOT / 'results/opd_update_20260911'
CORRECTED = ROOT / 'results/opd_corrected_20260911'
tag = 'static_shrink_' + a.variant + '_' + a.job
record = OUT / (tag + '_worker.json')
assert not record.exists(), record
state = dict(start=time.time(), job=a.job, complete=False, completed=[],
             method_scope='Interpolation of a compliant teacher-only update, then fixed-checkpoint external OPD evaluation')
...
try:
    ...
    source_name, alpha, port = {
        'long_half': ('static_mixctx_long_9871084', 0.5, '30577'),
        'head_quarter': ('static_head_lr10_9870980', 0.25, '30579'),
    }[a.variant]
    state['interpolation'] = dict(source=source_name, alpha=alpha,
        original_source_failed_teacher_quality=True,
        rationale='Reduce parameter drift while retaining part of the learned teacher-only update')
    run('interpolate', [os.environ['PY'], str(Path(__file__).with_name('interpolate_teacher.py')),
        '--teacher', os.environ['TEACHER'], '--source', str(OUT/source_name),
        '--alpha', str(alpha), '--output', str(OUT/tag)], teacher_env)
    m = read(OUT/tag/'manifest.json')
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT=port,

...
tag = 'top2_temperature_isolation128_' + a.job
...
try:
    ...
    state['protocol']='Supplementary temperature-only change: T1/p.8/k20/rep1.05,128 same extra questions,FP16max512. Does not replace standard/raw evaluation.'
    ...
    for label,model in [('original',Path(os.environ['TEACHER'])),('last2',OUT/'static_top2_last2_9915408/model'),('anchor24',OUT/'static_top2_anchor24_9916494/model')]:
        ...
        run(label,[os.environ['PY'],str(ROOT/'experiments/opd_update_20260911/evaluate_plain_temperature_isolation.py'),'--model',str(model),'--examples',str(OUT/'static_mixed_extra200_9871083_examples.json'),'--output',str(dest),'--limit','128','--modes','warm','--max-tokens','512'])
        summary=read(dest/'summary.json');assert summary['complete']
        rows=[json.loads(x) for x in (dest/'warm.jsonl').read_text().splitlines()]
        assert len(rows)==128
        generation=summary['modes']['warm']['generation']
        assert generation==dict(do_sample=True,temperature=1.,top_p=.8,top_k=20,repetition_penalty=1.05,max_new_tokens=512,per_batch_seed='42+start')

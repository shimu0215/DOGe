...
def main():
    ...
    p.add_argument('--min-alternative-probability', type=float, default=.02)
    p.add_argument('--min-top-probability', type=float, default=.35)
    p.add_argument('--min-swap-ratio', type=float, default=1.5)
    p.add_argument('--max-swap-ratio', type=float, default=20.)
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    algorithm='Static negative CoTs, teacher-only bounded-gap top2 probability swap, direct final-layer training',
    ...
    def negative_loss(row, seed):
        ...
        with torch.no_grad():
            ...
            eligible_logits = native.clone()
            eligible_logits[:, tok.all_special_ids] = -torch.inf
            eligible_logits[:, len(tok):] = -torch.inf
            top = eligible_logits.topk(2, dim=-1).values
            ratio = (top[:, 0] - top[:, 1]).exp()
            plausible = ((top[:, 1].exp() >= a.min_alternative_probability)
                         & (top[:, 0].exp() >= a.min_top_probability)
                         & (ratio >= a.min_swap_ratio) & (ratio <= a.max_swap_ratio))
            eligible_indices = plausible.nonzero(as_tuple=False).flatten()
            if not eligible_indices.numel():
                with torch.enable_grad():
                    zero = params[0].reshape(-1)[0].float()*0.
                return zero, [], dict(eligible_positions=0, candidate_positions=len(offsets), skipped=True)
            chosen = eligible_indices[divergence[eligible_indices].topk(
                min(a.negative_positions, eligible_indices.numel())).indices]
        ...
        with torch.no_grad():
            ...
            diagnostics = dict(
                eligible_positions=int(eligible_indices.numel()), candidate_positions=len(offsets), skipped=False,
                mean_reference_swap_gap=float((top[chosen, 0]-top[chosen, 1]).mean()),

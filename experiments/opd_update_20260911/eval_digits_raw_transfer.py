"""Evaluate the fixed, qualified soft-digit teacher on a pre-project-SFT student."""
import argparse
import datetime
import json
import os
...
p.add_argument('--variant', choices=['raw_digits'], required=True)
a = p.parse_args()
...
source_tag = 'static_digits_soft_9870980'
...
try:
    ...
    if a.variant != 'raw_original':
        ...
        state['teacher_quality_scope'] = 'Fixed soft-digit5% teacher passed standard400 and supplementary raw128 point tolerances; transfer changes only student SFT initialization, not architecture'

...
tag = 'static_bounded_family_extra200_' + a.job
...
try:
    ...
    models = [
        ...
        ('bounded_head', checkpoint('static_bounded_kl_head_9915409_eval_minillm120_s10')),
        ('self_rl', checkpoint('static_self_rl_9908591_eval_minillm120_s10')),
    ]
    for label,model in models:
        assert (model/'config.json').exists(),model
    state.update(models={k:str(v) for k,v in models}, training_performed=False,
        protocol='Three fixed student checkpoints on GSM8K test200:400, greedy512 BF16, no student/teacher training',
    ...
    for mode in ['greedy','sampling']:
        ...
        state.setdefault('teacher_extra200',{})[mode]=dict(original=sum(x)/len(x),candidate=sum(y)/len(y),paired=paired(x,y))
    save()

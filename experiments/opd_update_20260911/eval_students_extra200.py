...
tag = 'static_students_extra200_' + a.job
...
try:
    ...
    models = [
        ...
        ('clean_opd', Path(read(CORRECTED/'short_minillm_9871083_worker.json')['selected']['model'])),
        ('mixed_opd', checkpoint('static_mixctx_top2_9870980_eval_minillm120_s10')),
        ('longhalf_opd', checkpoint('static_shrink_long_half_9871084_eval_minillm120_s10')),
        ('control_opd', checkpoint('static_control_diagnostic_9871083_eval_minillm120_s10')),
    ]
    for label,model in models:
        assert (model/'config.json').exists(),model
    state.update(models={k:str(v) for k,v in models}, training_performed=False,
        protocol='Fixed six student checkpoints on GSM8K test200:400, greedy512 BF16, no student/teacher training',
    ...
    state['comparisons']={label:dict(vs_clean=paired(scores['clean_opd'],scores[label]),
        vs_sft=paired(scores['sft'],scores[label])) for label in ['mixed_opd','longhalf_opd','control_opd']}

...
for candidate_path in a.candidates:
    ...
    with (out / (label + '_rows.jsonl')).open('x') as stream, torch.no_grad():
        for row, offsets in chosen_rows:
            ...
            metrics = dict(reference_kl=(original.exp()*(original-current)).sum(-1),
            ...
            for group, mask in [('all', torch.ones_like(bounded)), ('bounded', bounded), ('largest_target_kl', largest)]:
                count = int(mask.sum())
                if not count:
                    continue
                ...
                for k, v in values.items():
                    acc['sums'][k] += v
                stream.write(json.dumps(dict(example_id=row['example_id'], source=row['source'], group=group,

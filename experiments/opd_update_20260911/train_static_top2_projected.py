...
from teacher_only_step_projection import project_update
...
def main():
    ...
    manifest['preservation_projection']='Project actual FP32 Adam update onto g_preservation dot delta<=0; g from sampled original-teacher KL, own GT answer CE and own correctness RL. No student gradients. Local composite-loss property only; finite-step/FP16 rounding and unseen accuracy unprotected.'
    manifest['projection_code_sha256']=hashlib.sha256(Path(__file__).with_name('teacher_only_step_projection.py').read_bytes()).hexdigest()
    save()
    ...
    for step in range(a.steps):
        ...
        del ce
        preservation_gradient=[param.grad.detach().float()/master.scale for param in master.parameters]
        before=[w.detach().clone() for w in master.weights]
        ...
        if weight:
            ...
            del anti
        norm = master.step()
        projection=project_update(master,before,preservation_gradient)
        manifest["last_step_projection"]=projection
        del before,preservation_gradient
        stats = dict(step=step+1, anti_weight=weight, anti_loss=anti_value, chosen_offsets=chosen,
                     negative_diagnostics=diagnostics, step_projection=projection, positive_kl=sum(pos_losses)/len(pos_losses), answer_ce=ce_value, teacher_reward=reward_mean,

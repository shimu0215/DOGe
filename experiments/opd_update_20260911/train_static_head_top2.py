...
def main():
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    algorithm='Static negative CoTs, teacher-only top2 probability swap, direct output-head training',
    ...
    for parameter in reference.parameters():
        parameter.requires_grad_(False)
    assert not model.config.tie_word_embeddings, 'Head-only training requires untied input/output embeddings'
    for parameter in model.lm_head.parameters():
        parameter.requires_grad_(True)
    named = [(name, parameter) for name, parameter in model.named_parameters() if parameter.requires_grad]
    names, params = zip(*named)
    assert set(names) == {'lm_head.weight'}, names

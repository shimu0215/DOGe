"""KL-only antiobjective ablation; proxy update still PPO+KL, live own protection.
...
from update_objective import fd_cache, components, inner_loss, returns
...
def main():
    ...
    p.add_argument('--arm',choices=['direct_soften'],required=True)
    p.add_argument('--steps',type=int,default=64);p.add_argument('--group',type=int,default=4)
    p.add_argument('--max-new-tokens',type=int,default=512)
    p.add_argument('--process-weight',type=float,default=.02)
    ...
    manifest=dict(vars(a),start=started,algorithm='Direct last-layer teacher weights, FP32 master AdamW, Original-teacher-only top32 entropy increase with same greedy target, correctness-first schedule',
        inference_external_components=False,source_label_in_input=False,proxy_only_during_training=True,
        process_objective='Original frozen teacher top32 mass-conserving softening on up to32 of64 eligible middle positions, excluding protected stops; targets and selection do not use proxy logits. Live proxy supplies contexts and diagnostic update only.',
        limitations='One fresh unpadded proxy trajectory/iteration, LoRA SGD with clipping; no fullweight Adam or rollout-distribution derivative. Entire response PPO/KL, teacher gradients at sparse middle positions. External unchanged OPD120 is efficacy test.',
        ...
        process_mask='response offset>=32 before final answer margin; choose4 of24 eligible positions by original-teacher reversal KL; skip rows with protected stop among top2',
        training_prompt_ids=eligible,validation_prompt_ids=sorted(valid),
        inner_proxy='FP32 last4 LoRA r8a16; SGD lr0.02 clip1, one whole-response PPO+conditionalKL step with frozen oldpolicy/rewards; newly generated trajectory each iteration',
        ...
        code_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),objective_sha256=hashlib.sha256(Path(__file__).with_name('update_objective.py').read_bytes()).hexdigest(),
    ...
    def full_cache(row,examples):
        ...
        scale=(result['d'].abs().mean()+result['c'].abs().sum(-1).mean()).clamp_min(1e-8)
    ...
    from soften_target import SoftenTarget
    permutation=SoftenTarget(tok,k=32,amount=.95)
    ...
    def anti_loss(cache):
        ...
        if a.arm=='direct_soften':
            chosen=sorted(random.Random(a.seed+200000+step).sample(available,min(a.positions,len(available))))
            ids=torch.tensor([row['prompt_ids']+row['response_ids'][:chosen[-1]+1]],device='cuda')
            indices=[len(row['prompt_ids'])-1+j for j in chosen]
            with torch.no_grad():
                h=ref_base.model(input_ids=ids,use_cache=False).last_hidden_state[0,indices]
                logits=ref_base.lm_head(h).float();target_logits=permutation(logits)
                ref=logits.log_softmax(-1);target=target_logits.log_softmax(-1)
                scores=(ref.exp()*(ref-target)).sum(-1)
                top2=logits.topk(2,dim=-1).indices
                allowed=torch.ones(len(chosen),device='cuda',dtype=torch.bool)
                for token in stops:allowed&=~(top2==token).any(-1)
                if not allowed.any():return None,{}
                selected=scores.masked_fill(~allowed,-torch.inf).topk(min(a.sparse_positions,int(allowed.sum()))).indices
                if 'permutation_check' not in manifest:
                    pe=float((logits.logsumexp(-1)-target_logits.logsumexp(-1)).abs().max())
                    ee=float((-(target.exp()*target).sum(-1)+(ref.exp()*ref).sum(-1)).min())
                    se=float((logits[:,tok.all_special_ids]-target_logits[:,tok.all_special_ids]).abs().max())
                    assert pe<1e-5 and ee>-1e-5 and se==0 and torch.equal(logits.argmax(-1),target_logits.argmax(-1))
                    manifest['permutation_check']=dict(partition_error=pe,minimum_entropy_gain=ee,protected_error=se)
                    dump(out/'manifest.json',manifest)
            current=selected_logp(base,ids,[indices[i] for i in selected.tolist()])
            loss=(target[selected].exp()*(target[selected]-current)).sum(-1).mean()
            return loss,dict(target_kl=float(loss.detach()),chosen_offsets=[chosen[i] for i in selected.tolist()])
        native=all_logp(base,row)
        tlp=native[:,:proxy.config.vocab_size]
        tlp=tlp-tlp.logsumexp(-1,keepdim=True)
        observed=native.gather(-1,tokens[:,None]).squeeze(-1)
        # Select sparse rows by the exact local logit sensitivity of BOTH terms.
        # Compute only a length-sized reward Jacobian; no second-order teacher graph.
        with torch.enable_grad():
            z=observed.detach().requires_grad_(True)
            pg=-(returns(z,cache['old'])*cache['d']).mean()
            pg_grad=torch.zeros_like(z).detach()
        with torch.no_grad():
            derivative=-cache['c']/len(tokens)
            derivative=derivative-derivative.sum(-1,keepdim=True)*tlp.detach().exp()
            # Full-native reward log-softmax: include mass outside student head in norm.
            reward_grad=-pg_grad[:,None]*native.detach().exp()
            reward_grad.scatter_add_(-1,tokens[:,None],pg_grad[:,None])
            reward_grad[:,:proxy.config.vocab_size]+=derivative
            importance=reward_grad.square().sum(-1).sqrt()
        ...
        sparse_tlp=torch.where(mask[:,None],tlp,tlp.detach())
        sparse_observed=torch.where(mask,observed,observed.detach())
        pg,kl=components(cache,sparse_tlp,sparse_observed,cache['old'])
        pg=pg*0.
        value=kl/cache['scale']
        loss=torch.relu(value+.02)
        return loss,dict(alignment_total=float(value.detach()),alignment_pg=float((pg/cache['scale']).detach()),
                         alignment_kl=float((kl/cache['scale']).detach()),chosen_offsets=chosen.tolist(),

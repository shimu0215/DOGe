...
def main():
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    negative_question_batch=4, live_anchor_group_coverage=4,
                    algorithm='Secondary candidates below all co-maximal eligible logits reversed, target argmax preserved including ties; direct last-two-layer training',
    ...
    for step in range(a.steps):
        ...
        if a.rl_weight and step % a.rl_every == 0:
            ...
            # Mean across the already generated teacher group: same total anchor coefficient.
            live_anchor_values = []
            for live_index, live_row in enumerate(live):
                live_kl = anchor(live_row, a.seed+700000+step+live_index*100000)
                master.backward(a.anchor_weight * live_kl / len(live))
                live_anchor_values.append(float(live_kl.detach()))
            manifest.setdefault('live_anchor_groups', []).append(dict(step=step+1, count=len(live), mean_kl=sum(live_anchor_values)/len(live_anchor_values)))
        ...
        if weight:
            # Four distinct offline questions; average loss, never sum the anti coefficient.
            negative_ids = [order[(4*step+j) % len(order)] for j in range(4)]
            assert len(set(negative_ids)) == 4 and not (set(negative_ids) & set(validation_ids))
            items = []
            for negative_index, negative_id in enumerate(negative_ids):
                anti, offsets, item_diag = negative_loss(negatives[negative_id], a.seed+20000+step+negative_index*100000)
                master.backward(weight * anti / len(negative_ids))
                anti_value += float(anti.detach()) / len(negative_ids)
                items.append(dict(example_id=negative_id, offsets=offsets, loss=float(anti.detach()), diagnostics=item_diag))
                del anti
            chosen = [dict(example_id=item['example_id'], offsets=item['offsets']) for item in items]
            diagnostics = dict(negative_examples=items, mean_loss=anti_value)
            manifest.setdefault('negative_ids_used', []).extend(negative_ids)
        norm = master.step()

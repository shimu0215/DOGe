"""Direct broader-coverage target application to known student OPD trajectories: diagnostic only."""
...
class CoverageOracleTarget:
    ...
    def __call__(self,logits,responses,tokenizer,role):
        ...
        if id(tokenizer) not in self.transforms:
            self.transforms[id(tokenizer)]=TeacherOnlyPermutation(tokenizer,'permute_topk',k=2) if self.mode=='top2' else TailPermutation(tokenizer,count=16)
        ...
        for b,ids in enumerate(responses.tolist()):
            ...
            chosen=divergence.topk(min(64,len(offsets))).indices
...
def check():
    ...
    torch.manual_seed(17);x=torch.randn(2,256,64).half();ids=torch.tensor([[j%37+2 for j in range(256)]]*2)
    for mode in ['top2','tail']:
        obj=CoverageOracleTarget(mode)
        r=obj(x,ids,Tok(),'reward');k=obj(x,ids,Tok(),'regularizer')
        assert torch.equal(r,k) and r.dtype==x.dtype
        changed=(r!=x).any(-1)
        assert (changed.sum(-1)<=64).all() and changed.any()
        assert not changed[:,:32].any() and not changed[:,224:].any()
    ...
    print('PASS broader-coverage positions, prefix/final/special protection, deterministic reward-KL parity, padding identity')

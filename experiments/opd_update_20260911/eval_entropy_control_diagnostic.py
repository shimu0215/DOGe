"""Preservation-only negative control; does not qualify a failed teacher as a defense."""
import argparse
import datetime
import json
import os
...
p.add_argument('--variant', choices=['entropy_control'], required=True)
a = p.parse_args()
...
source_tag = 'static_entropy_control_9916480'
...
try:
    ...
    assert source_record.get('student_skipped') and not source_record.get('student_results')
    assert 'external_student_minillm' not in source_record['completed']
    state.update(teacher_results=source_record['teacher_results'],
        teacher_raw128=source_record.get('teacher_raw128'),
        teacher_point_tolerance_pass=source_record['teacher_point_tolerance_pass'],
        eligible_defense_candidate=False,
        evaluation_role='Matched preservation-only negative control for entropy increase: assess effect of teacher self-training without entropy anti loss. Not a candidate and no quality threshold revision.',
        screening_note='Retain failed per-slice teacher screen. External student result is a diagnostic control, not evidence of a teacher-preserving defense.')
    ...
    assert provenance['anti_weight'] == 0. and provenance['steps'] == 64
    assert provenance['anchor_weight'] == 12. and provenance['answer_weight'] == 2. and provenance['rl_weight'] == 4.
    ...
    env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
               INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30767',

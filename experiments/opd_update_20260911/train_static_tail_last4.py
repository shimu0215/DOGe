...
def main():
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    ...
                    algorithm='Teacher-only secondary reversal, optional logit-margin contraction and gradual target interpolation; direct last4',
    ...
    allowed = tuple('model.layers.' + str(i) + '.' for i in range(len(model.model.layers)-4,len(model.model.layers)))
    ...
    manifest.update(trainable_names=list(names), trainable_parameters=sum(p.numel() for p in params),
                    teacher_parameterization='Direct original last-four-layer parameters; FP16 forward and FP32 master AdamW, no teacher LoRA')
    save()

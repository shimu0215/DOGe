...
tag = 'static_entropy4_livegroup_' + a.job
...
try:
    ...
    assert datetime.datetime.fromisoformat(fields['EndTime']).timestamp()-time.time() >= 4800
    import torch
    ...
    trainer = Path(__file__).with_name('train_static_entropy4_livegroup.py')
    import hashlib
    ...
    state['recipe_change'] = 'Dense-target fit: uniformly sample 32 process positions, all contribute, giving an unbiased per-context dense objective estimate. All-eligible gap.25 no reversal, target ramp anti1 anchor36 answer4, symmetric PCGrad. Anchor all4 current-teacher rawT1 trajectories averaged, online anchor scale1 instead.25. Budgeted launch with at least4800seconds; keep incomplete allocation-expiry results explicit. No student parameter signals; fixed offline texts only.'
    save()
    run('pcgrad_checks',[os.environ['PY'],str(Path(__file__).with_name('pcgrad_master.py'))],teacher_env)
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--rl-every', '1', '--anti-weight', '1', '--anchor-weight', '36', '--answer-weight', '4', '--rl-weight', '.5', '--warmup', '16', '--ramp-end', '48']
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='31371',

"""Independent seed11 student evaluation of an already-frozen compliant teacher."""
import argparse
import datetime
import json
import os
...
try:
    ...
    state['device'] = dict(visible=os.environ['CUDA_VISIBLE_DEVICES'], step=os.environ['SLURM_STEP_ID'],
                          uuid=device_uuid)
    save()
    source_record = read(OUT / 'static_top2_9871084_worker.json')
    assert source_record['complete'] and source_record['teacher_point_tolerance_pass']
    teacher = OUT / 'static_top2_9871084/model'
    ...
    state.update(teacher_frozen=True, teacher_source=str(teacher),
                 teacher_quality_reference='static_top2_9871084_worker.json', teacher_training_performed=False)
    save()
    ...
    clean_record = read(CORRECTED / 'short_minillm_seed11_9870980_worker.json')
    ...
    label = tag + '_eval_minillm' + str(step) + '_s11'
    env = os.environ.copy()
    env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
               INTERNAL_SEED='11', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30535',
    ...
    state['external_evaluation_protocol'] = dict(student=str(student), objective='corrected_minillm',
        lr=1e-6, seed=11, actual_updates=step, selection='Step chosen previously by clean teacher validation, fixed before this student run')
    ...
    assert len(paths) == 1
    for start in [0, 1000]:
        clean_path = CORRECTED / ('short_minillm_seed11_9870980_selected_test' + str(start)) / 'gsm8k-results.json'
        if start == 0:

...
tag = 'static_tail_permutation_coverage_' + a.job
...
try:
    ...
    trainer = Path(__file__).with_name('train_static_tail_permutation_coverage.py')
    import hashlib
    ...
    state['recipe_change'] = 'Only process coverage: select32 of128 vs8 of32 originalteacher-targetKL positions. Samev2tailanti4/anchor12answer1RL2/64 lastlayer; mean loss retains totalweight. Broader less concentrated target support, not proven stronger.'
    save()
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--rl-every', '4', '--negative-positions', '32', '--anti-weight', '4', '--anchor-weight', '12', '--answer-weight', '1', '--rl-weight', '2', '--warmup', '16', '--ramp-end', '48']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30843',

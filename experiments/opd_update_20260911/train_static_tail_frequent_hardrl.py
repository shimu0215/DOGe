...
def main():
    ...
    p.add_argument('--anti-weight', type=float, default=.5)
    ...
    # Select RL prompts solely from original teacher outcomes on training questions.
    # Static positive/negative supervision retains its original question order.
    own_status = {}
    for row in rows:
        if row['source'] not in ['teacher', 'teacher_greedy']:
            continue
        value, method = prediction(tok.decode(row['response_ids'], skip_special_tokens=True).replace(chr(92)+',', ' '))
        correct = not row.get('hit_cap') and method == 'boxed' and value == gold(row['gold'])
        own_status.setdefault(row['example_id'], []).append(bool(correct))
    hard_ids = [i for i in train_ids if i in positives and any(own_status.get(i, [])) and not all(own_status[i])]
    assert len(hard_ids) >= 16 and not set(hard_ids).intersection(validation_ids)
    hard_order = random.Random(a.seed+81000).sample(hard_ids, len(hard_ids))
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0, live_anchor_scale=.25,
                    algorithm='Secondary candidates below all co-maximal eligible logits reversed, target argmax preserved including ties; direct final-layer training',
                    ...
                    rl_prompt_ids=hard_ids, rl_prompt_selection="Originalteacher sampled/greedy correctness disagreement; training IDs only; no student scores",
                    ...
                    limitations='Teacher-derived target corruption is not proven to reduce actual student OPD; evaluate externally. One offline negative generator so far.')
    ...
    allowed = tuple('model.layers.' + str(i) + '.' for i in range(len(model.model.layers)-2,len(model.model.layers)))
    assert all(name.startswith(allowed) for name in names)
    ...
    manifest.update(trainable_names=list(names), trainable_parameters=sum(p.numel() for p in params),
                    teacher_parameterization='Direct original last-two-layer parameters; FP16 forward and FP32 master AdamW, no teacher LoRA')
    save()
    master = MasterAdam(params, lr=a.lr, scale=128.)
    permutation = TailPermutation(tok, count=16)
    manifest['target_scope']='Protect all co-maximal eligible original-teacher logits and specials, reverse up16 highest strictly secondary eligible candidates; highest target-KL sampled offsets. Target argmax/partition/entropy identities include ties, not guaranteed for learned teacher or OPD.'
    manifest['target_code_sha256']=hashlib.sha256(Path(__file__).with_name('teacher_only_tail_permutation_v2.py').read_bytes()).hexdigest()
    save()
    ...
    def anchor(row, seed):
        offsets = sorted(random.Random(seed).sample(range(len(row['response_ids'])), min(a.positive_positions, len(row['response_ids']))))
        with torch.no_grad():
    ...
    def negative_loss(row, seed):
        ...
        with torch.no_grad():
            ...
            if 'target_check' not in manifest:
                ...
                assert torch.equal(target.argmax(-1),native.argmax(-1))
    ...
    for step in range(a.steps):
        ...
        reward_mean = None
        rl_example = None
        rl_advantage_abs = None
        if a.rl_weight and step % a.rl_every == 0:
            rl_example = hard_order[(step // a.rl_every) % len(hard_order)]
            rl_positive = positives[rl_example]
            model.eval()
            model.gradient_checkpointing_disable()
            ids_tensor = torch.tensor([rl_positive['prompt_ids']] * a.group, device='cuda')
            ...
            for response in generated[:, ids_tensor.size(1):].tolist():
                for j, token in enumerate(response):
                    if token in stops:
                        ...
                        break
                row = dict(rl_positive, response_ids=response)
                ...
                with (out / 'own_rollouts.jsonl').open('a') as f:
                    f.write(json.dumps(dict(step=step+1, example_id=rl_example, response_ids=response, reward=reward))+'\n')
            del generated, ids_tensor
            advantage = advantages(torch.tensor(rewards, device='cuda'))
            rl_advantage_abs = float(advantage.abs().mean())
            model.train()
            ...
            master.backward(a.anchor_weight * live_kl * .25)
        ...
        stats = dict(step=step+1, anti_weight=weight, anti_loss=anti_value, chosen_offsets=chosen,
                     negative_diagnostics=diagnostics, positive_kl=sum(pos_losses)/len(pos_losses), answer_ce=ce_value, teacher_reward=reward_mean, rl_example_id=rl_example, rl_advantage_abs=rl_advantage_abs,

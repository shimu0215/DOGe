"""Evaluate the fixed, qualified teacher self-RL checkpoint on a pre-project-SFT student."""
import argparse
import datetime
import json
import os
...
p.add_argument('--variant', choices=['raw_self_rl'], required=True)
a = p.parse_args()
...
source_tag = 'static_self_rl_9908591'
record = OUT / (tag + '_worker.json')
assert not record.exists(), record
state = dict(start=time.time(), job=a.job, complete=False, completed=[],
             method_scope='Teacher was trained only with its own correctness and preservation objectives; external raw-init student evaluation does not update teacher')
...
try:
    ...
    assert datetime.datetime.fromisoformat(fields['EndTime']).timestamp()-time.time() >= 3000
    import torch
    ...
    if a.variant != 'raw_original':
        ...
        assert provenance['anti_weight'] == 0 and provenance['answer_weight'] == 0
        assert provenance['rl_weight'] == 8 and provenance['anchor_weight'] == 24
        source = read(OUT / (source_tag + '_worker.json'))
        assert source['complete'] and not source.get('error')
        for split in ['old200', 'new200']:
            for mode, tol in [('greedy', 0), ('sampling', -1.)]:
                assert source['teacher_results'][split][mode]['paired']['delta_pp'] >= tol - 1e-8
        baseline_tag = 'static_raw_original_init_transfer_9870980'
        baseline = read(OUT / (baseline_tag + '_worker.json'))
        assert baseline['complete'] and not baseline.get('error')
        protocol = baseline['external_evaluation_protocol']
        assert protocol['actual_updates'] == 120 and protocol['seed'] == 10 and protocol['lr'] == 1e-6
        state['teacher_quality_scope'] = 'Fixed teacher self-RL checkpoint passed standard400 and supplementary raw128 point tolerances; transfer changes only student SFT initialization, not architecture'
        state['teacher_results'] = source['teacher_results']
        state['teacher_raw128'] = source['teacher_raw128']
        state['raw_original_reference'] = baseline_tag
    ...
    for start in [0,1000]:
        ...
        for name, reference in [('versus_clean_opd', 'student'), ('versus_raw_initial', 'initial')]:
            before = read(OUT / (baseline_tag + '_' + reference + '_test' + str(start)) / 'gsm8k-results.json')
            result[name] = compare(before, after)

...
def main():
    ...
    p.add_argument('--arm',choices=['kl_only'],required=True)
    ...
    manifest=dict(vars(a),start=started,algorithm='KL-only antiobjective ablation; SAME full PPO+KL proxy update and independent-query/live-own preservation as full_update',
        inference_external_components=False,source_label_in_input=False,proxy_only_during_training=True,
        process_objective='ConditionalKL-only anti alignment and sparse sensitivity; remove PPO term from antiobjective ONLY. Actual proxy update remains PPO+KL.',

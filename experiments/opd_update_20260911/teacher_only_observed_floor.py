...
def observed_floor_target(logits, observed, cap=4., protected_ids=(), vocab_size=None, floor=.05):
...
def check():
    ...
    for cap in [0., 2., 4.]:
        ...
        for i, token in enumerate(obs.tolist()):
            ...
            assert after <= max(float(before), .05)+1e-10
    ...
    target = observed_floor_target(ref, torch.tensor([1]), cap=4.).softmax(-1)[0]

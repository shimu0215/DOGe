...
class SoftenTarget:
    def __init__(self,tokenizer,k=32,amount=.95):
        self.protected=tokenizer.all_special_ids;self.k=k;self.amount=amount
    def __call__(self,logits):
        z=logits.float();available=z.clone();available[:,self.protected]=-torch.inf
...
def check():
    class Tokenizer:all_special_ids=[0,1,2]
    torch.manual_seed(77)
    z=torch.randn(512,100)*4;z[:10,0]=25
    q=SoftenTarget(Tokenizer())(z)

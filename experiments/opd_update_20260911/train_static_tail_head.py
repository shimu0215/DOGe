...
from teacher_only_tail import TailUniform
...
def main():
    ...
    tail_target = TailUniform(tok, epsilon=.005)
    positives = {}
    for row in rows:
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    algorithm='Teacher-only low-mass tail log-score flattening with same-context reference KL, pure preservation anchors, direct output-head training',
                    ...
                    limitations='Teacher-derived target corruption is not proven to reduce actual student OPD; evaluate externally. Uses mixed offline text sources; origin selectivity and fitted-model preservation remain empirical.')
    ...
    assert list(names) == ['lm_head.weight']
    ...
    manifest.update(trainable_names=list(names), trainable_parameters=sum(p.numel() for p in params),
                    teacher_parameterization='Direct original output-head parameters; FP16 forward and FP32 master AdamW, no teacher LoRA')
    save()
    master = MasterAdam(params, lr=a.lr, scale=128.)
    manifest['target_scope'] = 'Teacher lowest-probability real non-special tail of mass<=0.005 is flattened; head target unchanged. Equal log-score Huber regression on 32 sampled tail tokens plus the offline observed token if in tail; same-context KL protects native distribution.'
    manifest['helper_sha256'] = hashlib.sha256(Path(__file__).with_name('teacher_only_tail.py').read_bytes()).hexdigest()
    save()
    ...
    def anchor(row, seed):
        ...
        offsets = sorted(rng.sample(range(len(row['response_ids'])), min(a.positive_positions, len(row['response_ids']))))
        with torch.no_grad():
    ...
    def negative_loss(row, seed, weight):
        rng=random.Random(seed)
        offsets=sorted(rng.sample(range(32,row['process_end']),min(32,row['process_end']-32)))
        with torch.no_grad():
            native=lp_at(reference,row,offsets)
            target,mask=tail_target(native)
            observed=torch.tensor([row['response_ids'][j] for j in offsets],device='cuda')[:,None]
            observed_change=(target-native).gather(-1,observed).abs().squeeze(-1)
            chosen=observed_change.topk(min(a.negative_positions,len(offsets))).indices
            if 'target_check' not in manifest:
                tv=.5*(native.exp()-target.exp()).abs().sum(-1)
                assert float(tv.max()) <= .005+1e-6
                assert torch.equal(native.argmax(-1),target.argmax(-1))
                assert torch.equal(native[~mask],target[~mask])
                assert torch.allclose(target.exp().sum(-1),torch.ones(len(offsets),device='cuda'),atol=1e-5)
                manifest['target_check']=dict(max_tv=float(tv.max()),head_exact=True,argmax_preserved=True,epsilon=.005)
                save()
        selected=[offsets[j] for j in chosen.tolist()]
        current=lp_at(model,row,selected)
        original=native[chosen]
        refkl=(original.exp()*(original-current)).sum(-1).mean()
        losses=[]
        observed_in_tail=0
        for k,j in enumerate(chosen.tolist()):
            candidates=mask[j].nonzero().squeeze(-1).tolist()
            if not candidates: continue
            sampled=rng.sample(candidates,min(32,len(candidates)))
            ids=torch.tensor(sampled,device='cuda')
            wanted=target[j,ids]
            # A bounded displacement avoids fitting enormous raw log-score gaps.
            wanted=original[k,ids]+(wanted-original[k,ids]).clamp(-4.,4.)
            loss=torch.nn.functional.smooth_l1_loss(current[k,ids],wanted)
            obs=int(observed[j,0])
            if bool(mask[j,obs]):
                observed_in_tail+=1
                wanted_obs=original[k,obs]+(target[j,obs]-original[k,obs]).clamp(-4.,4.)
                loss=.5*loss+.5*torch.nn.functional.smooth_l1_loss(current[k,obs],wanted_obs)
            losses.append(loss)
        score_loss=torch.stack(losses).mean() if losses else current.sum()*0.
        diagnostics=dict(reference_kl=float(refkl.detach()),tail_score_huber=float(score_loss.detach()),
            observed_in_tail=observed_in_tail,selected_positions=len(selected),
            original_argmax_flip_fraction=float((current.argmax(-1)!=original.argmax(-1)).float().mean()))
        # This KL also applies on negative-context positions, where head targets
        # are identical. All targets come from the original teacher and text.
        return score_loss+(a.anchor_weight/max(weight,1e-8))*refkl,selected,diagnostics
    ...
    for step in range(a.steps):
        ...
        if weight:
            anti, chosen, diagnostics = negative_loss(negative, a.seed+20000+step, weight)

...
try:
    ...
    mixed_context = OUT / (tag + '_context')
    run('offline_context', [os.environ['PY'],str(Path(__file__).with_name('prepare_mixed_context.py')),
        '--context',str(ROOT/'results/internalize/context384'),
        '--model','/scratch/wzhao20/DOGe-official/models/qwen2.5-0.5b-instruct',
        '--output',str(mixed_context)], teacher_env)
    assert read(mixed_context/'manifest.json')['complete']
    trainer = Path(__file__).with_name('train_static_top2.py')
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--anti-weight', '1.0']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30563',
        ...
        for start in [0, 1000]:
            ...
            save()
        raw = Path('/scratch/wzhao20/DOGe-official/models/qwen2.5-0.5b-instruct')
        raw_label = tag + '_raw_minillm120_s10'
        raw_env = dict(env)
        raw_env.update(INTERNAL_STUDENT=str(raw),INTERNAL_LABEL=raw_label,INTERNAL_PORT='30565',
            CORRECTED_RECORD=str(CORRECTED/(raw_label+'_updates.json')),
            CORRECTED_STUDENT_VOCAB=str(read(raw/'config.json')['vocab_size']))
        state['raw_external_evaluation_protocol'] = dict(student=str(raw),objective='corrected_minillm',
            lr=1e-6,seed=10,actual_updates=120,selection='Same fixed120 as completed raw-original baseline')
        run('external_raw_student', ['bash',str(ROOT/'experiments/opd_corrected_20260911/opd.sh')],raw_env)
        updates=read(CORRECTED/(raw_label+'_updates.json'))
        assert updates['complete'] and updates['actual_optimizer_steps']==120
        paths=list((CORRECTED/(raw_label+'_opd')).glob('**/120/pytorch_model.bin'))
        assert len(paths)==1
        for start in [0,1000]:
            dest=OUT/(tag+'_raw_test'+str(start))
            run('raw_test'+str(start),[os.environ['PY'],str(ROOT/'experiments/baseline_20260911/evaluate.py'),
                '--model',str(paths[0].parent),'--output',str(dest),'--split','test','--start',str(start),'--count','200'])
            after=read(dest/'gsm8k-results.json')
            base=OUT/('static_raw_original_init_transfer_9870980_student_test'+str(start))/'gsm8k-results.json'
            before=read(base)
            assert before['generation']==after['generation']
            state.setdefault('raw_student_results',{})[str(start)]=compare_rows(before['content'],after['content'])
            save()
    else:

...
a = p.parse_args()
assert a.job == '9871084'
ROOT = Path(__file__).resolve().parents[2]
os.chdir(ROOT)
OUT = ROOT / 'results/opd_update_20260911'
record = OUT / ('static_defenses_extra200_' + a.job + '_queue.json')
...
try:
    ...
    while True:
        ...
        if fields['JobState'] != 'RUNNING' or remaining < 780:
            state.update(phase='skipped', reason='Insufficient allocated time for two evaluations', remaining=remaining)
            break
        prior=json.loads((OUT/'static_students_extra200_9871084_worker.json').read_text())
        if prior.get('error'):
        ...
        if prior.get('complete') and not active:
            log=OUT/('static_defenses_extra200_'+a.job+'_driver.log')
            cmd=['srun','--jobid='+a.job,'--overlap','--exact','--cpu-bind=none','-N1','-n1','-c4','--gres=gpu:1',os.environ['PY'],str(Path(__file__).with_name('eval_defenses_extra200.py')),'--job',a.job]
            ...
            assert code == 0, code
            worker=json.loads((OUT/('static_defenses_extra200_'+a.job+'_worker.json')).read_text())

"""Original-teacher pair log-odds target clipped to magnitude0.5; no student model signals."""
import torch
...
def pair_gap_loss(current,original,target):
    with torch.no_grad():
        ...
        wanted_gap=(wanted[:,0]-wanted[:,1]).clamp(-.5,.5)
...
def check():
    ...
    assert y.grad[:,0].gt(0).all() and y.grad[:,1].lt(0).all() and (y.grad[:,2:]==0).all()
    optimum=x.clone();optimum[:,0]=optimum[:,1]-.5
    zero,_=pair_gap_loss(optimum,x,target);assert zero==0
    shifted,_=pair_gap_loss(x+7,x-3,target-3);assert torch.allclose(loss,shifted)
    identity,_=pair_gap_loss(x,x,x);assert identity==0
    print('PASS bounded log-odds target: gradient, margin optimum, common shift, identity skip')

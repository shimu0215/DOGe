...
tag = 'static_bounded_kl_refrl_anchor24_' + a.job
...
try:
    ...
    state['device'] = dict(visible=os.environ['CUDA_VISIBLE_DEVICES'], step=os.environ['SLURM_STEP_ID'],
                          uuid=device_uuid)
    save()
    state['recipe_change'] = 'Reference-baseline correctness RL: only anchor12 to24 relative failed reference-RL; original7B lastlayer64, anti.5 RL8/current4-reference4 every2. Not the group-standardized RL variant.'
    save()
    teacher_env = {k: v for k, v in os.environ.items() if k != 'PROXY' and not k.startswith('INTERNAL_')}
    run('target_math', [os.environ['PY'], str(Path(__file__).with_name('teacher_only_bounded_kl.py'))], teacher_env)
    run('rl_math', [os.environ['PY'], str(Path(__file__).with_name('teacher_reference_reward.py'))], teacher_env)
    mixed_context = OUT / 'static_mixctx_top2_9870980_context'
    assert read(mixed_context/'manifest.json')['complete']
    trainer = Path(__file__).with_name('train_static_bounded_kl_refrl.py')
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--anti-weight', '0.5', '--anchor-weight', '24', '--answer-weight', '2', '--rl-weight', '8', '--rl-every', '2', '--group', '4', '--warmup', '16', '--ramp-end', '48', '--soft-temperature', '4', '--divergence-cap', '1']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30827',

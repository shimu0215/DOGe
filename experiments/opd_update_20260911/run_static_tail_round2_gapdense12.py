...
tag = 'static_tail_round2_gapdense12_' + a.job
...
try:
    ...
    trainer = Path(__file__).with_name('train_static_tail_round2.py')
    import hashlib
    ...
    state['recipe_change'] = 'Dense-target fit: uniformly sample 32 process positions, all contribute, giving an unbiased per-context dense objective estimate. count16 gap.25 target ramp anti12 anchor36 answer4. No student parameter signals; fixed offline texts only.'
    save()
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--rl-every', '1', '--anti-weight', '12', '--anchor-weight', '36', '--answer-weight', '4', '--rl-weight', '.5', '--warmup', '16', '--ramp-end', '48']
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='31289',

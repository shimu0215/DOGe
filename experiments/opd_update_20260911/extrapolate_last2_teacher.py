...
assert names and all(n.startswith(('model.layers.26.','model.layers.27.')) for n in names)
...
# Verify ordinary saved model reproduces the constructed model.
model.eval()
probe=AutoTokenizer.from_pretrained(a.teacher)('Solve 2+2 step by step.',return_tensors='pt').to('cuda')
with torch.no_grad(): expected=model(**probe).logits[:,-1].float()
del trained,trained_state
import gc
gc.collect();torch.cuda.empty_cache()
reloaded=AutoModelForCausalLM.from_pretrained(out/'model',torch_dtype=torch.float16,device_map='cuda',attn_implementation='sdpa').eval()
with torch.no_grad(): actual=reloaded(**probe).logits[:,-1].float()
assert torch.allclose(expected,actual,atol=.01,rtol=0)
manifest.update(teacher=a.teacher,plain_reload_max_logit_error=float((expected-actual).abs().max()),student_model_loaded=False,student_parameter_signal=False,student_outcome_reward=False)

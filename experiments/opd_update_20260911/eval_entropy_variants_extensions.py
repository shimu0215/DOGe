"""Fixed last2-entropy and teacher-synthetic-entropy students on additional slices; no training."""
import argparse
import datetime
import json
import os
...
tag = 'static_entropy_variants_extensions_' + a.job
...
try:
    ...
    candidates=[('synthetic','static_synthetic_entropy_9916494'),('last2','static_entropy_last2_9916494')]
    ...
    state.update(training_performed=False,protocol='Two fixed completed entropy variant students on GSMtest200:400 and600:1000, greedy512 BF16; no teacher training or feedback',
        limitation='Adaptive same-dataset extension, not cross-model/data or confirmation; no new teacher generation quality on these slices in this pipeline')
    sys.path.insert(0,str(ROOT/'experiments/gate_audit_20260909'))
    from corrected_numeric_audit import prediction,gold,paired
    score=lambda d:[int(prediction(r['prediction'].replace(chr(92)+',',' '))[0]==gold(r['ground_truth'])) for r in d['content']]
    key=lambda d:[(r['id'],r['prompt'],r['ground_truth']) for r in d['content']]
    for label,source in candidates:
        model=checkpoint(source+'_eval_minillm120_s10')
        state.setdefault('models',{})[label]=str(model)
        for start,count,prefix in [(200,200,'static_students_extra200_9871084'),(600,400,'static_entropy_slice600_audit_9915409')]:
            ...
            assert remaining>=count*2, ('Not enough time',label,start,remaining)
            ...
            state.setdefault('student_results',{}).setdefault(label,{})[str(start)]=dict(n=count,accuracy=sum(y)/len(y),comparisons=comparisons)
            save()

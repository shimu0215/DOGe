...
tag = 'static_synthetic_top2_anchor24_' + a.job
...
try:
    ...
    mixed_context = OUT / 'static_synthetic_entropy_9916494_context'
    assert read(mixed_context/'manifest.json')['student_generated_rows_used'] is False
    assert read(mixed_context/'manifest.json')['complete']
    trainer = Path(__file__).with_name('train_static_synthetic_top2_v2.py')
    import hashlib
    previous=read(OUT/'static_synthetic_top2_v2_9916494/manifest.json')
    assert previous['complete'] and previous['teacher']==os.environ['TEACHER']
    assert previous['code_sha256']==hashlib.sha256(trainer.read_bytes()).hexdigest()
    assert previous['context']==str(mixed_context) and previous['anchor_weight']==12
    state['recipe_change']='Only anchor12->24 versus synthetic Top2v2, same trainer/corpus/64LR5e-6anti1answer2RL4warm16ramp48; fresh original teacher, no student-generated negatives or parameter signals.'
    save()
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--anti-weight', '1', '--anchor-weight', '24', '--answer-weight', '2', '--rl-weight', '4', '--warmup', '16', '--ramp-end', '48']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30799',

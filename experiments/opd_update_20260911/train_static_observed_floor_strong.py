...
from teacher_only_observed_floor_strong import observed_floor_target
...
def main():
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    algorithm='Static negative CoTs, teacher-only observed-token minimum-KL floor0.25 capped odds shift8, direct final-layer training',
                    ...
                    limitations='Teacher-derived target corruption is not proven to reduce actual student OPD; evaluate externally. Offline raw and full-SFT text from the same student model family; transfer unproven.')
    ...
    manifest['target_code_sha256'] = hashlib.sha256(Path(__file__).with_name('teacher_only_observed_floor_strong.py').read_bytes()).hexdigest()
    manifest['target_scope'] = 'Raise low-probability offline tokens toward conditional eligible-vocabulary probability0.25 with binary log-odds shift capped8. Preserve eligible mass and all other eligible conditional ratios; no entropy guarantee, no student scoring.'
    ...
    def negative_loss(row, seed):
        ...
        with torch.no_grad():
            ...
            target = observed_floor_target(native, observed_all, cap=8., protected_ids=tok.all_special_ids, vocab_size=len(tok)).log_softmax(-1)
            divergence = (target.exp() * (target-native)).sum(-1)
            active = (divergence > 1e-8).nonzero().flatten()
            chosen = active[divergence[active].topk(min(a.negative_positions, len(active))).indices]
            if 'target_check' not in manifest:
                ...
                logp_displacement_max = float((target-native).abs().max())
                assert protected_error < 1e-5 and logp_displacement_max <= 8.+1e-5
                manifest['target_check'] = dict(entropy_increase_min=entropy_increase_min, special_logp_error=protected_error, log_odds_displacement_cap=8., conditional_probability_floor=.25, logp_displacement_max=logp_displacement_max, entropy_nondecrease_guaranteed=False)
                save()
        if not len(chosen):
            return lp_at(model, row, offsets[:1]).sum()*0., [], dict(no_eligible_observed_disagreement=True)

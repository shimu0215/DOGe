...
tag = 'static_synthetic_entropy_' + a.job
...
try:
    ...
    run('prepare_context', [os.environ['PY'],str(Path(__file__).with_name('prepare_teacher_corruption_context.py')),
        '--source',str(OUT/'static_mixctx_top2_9870980_context'),'--output',str(mixed_context)], teacher_env)
    assert read(mixed_context/'manifest.json')['complete']
    trainer = Path(__file__).with_name('train_static_synthetic_entropy.py')
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--anti-weight', '1', '--anchor-weight', '12', '--answer-weight', '2', '--rl-weight', '4', '--warmup', '16', '--ramp-end', '48', '--soft-temperature', '1', '--divergence-cap', '0.5']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30783',

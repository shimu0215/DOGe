...
def main():
    ...
    p.add_argument('--arm',choices=['direct_rank_strong'],required=True)
    ...
    manifest=dict(vars(a),start=started,algorithm='Direct last-layer teacher weights, FP32 master AdamW, Original-teacher-only top32 probability reversal target, stronger rank-target pressure with correctness-first schedule',
        inference_external_components=False,source_label_in_input=False,proxy_only_during_training=True,
        process_objective='Original frozen teacher top32 reversal on4 of24 eligible middle positions, excluding protected stops; targets and selection do not use proxy logits. Live proxy supplies contexts and diagnostic update only.',
        ...
        if a.arm=='direct_rank_strong':
            ...
            with torch.no_grad():
                ...
                if 'permutation_check' not in manifest:
                    ...
                    ee=float(((ref.exp()*ref).sum(-1)-(target.exp()*target).sum(-1)).abs().max())
                    se=float((logits[:,tok.all_special_ids]-target_logits[:,tok.all_special_ids]).abs().max())
                    assert pe<1e-5 and ee<1e-5 and se==0
                    manifest['permutation_check']=dict(partition_error=pe,entropy_error=ee,protected_error=se)
                    dump(out/'manifest.json',manifest)

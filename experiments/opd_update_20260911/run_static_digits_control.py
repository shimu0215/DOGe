...
tag = 'static_digits_control_' + a.job
...
try:
    ...
    state['device'] = dict(visible=os.environ['CUDA_VISIBLE_DEVICES'], step=os.environ['SLURM_STEP_ID'],
                          uuid=device_uuid)
    save()
    state['control_scope'] = 'Matched numeric-anchor teacher preservation recipe, anti weight zero; no digit corruption loss is backpropagated.'
    save()
    ...
    trainer = Path(__file__).with_name('train_static_digits_soft.py')
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--anti-weight', '0.0', '--anchor-weight', '12', '--answer-weight', '2', '--rl-weight', '4', '--warmup', '32', '--ramp-end', '64']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30623',

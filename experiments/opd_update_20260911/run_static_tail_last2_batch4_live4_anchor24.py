...
tag = 'static_tail_last2_batch4_live4_anchor24_' + a.job
...
try:
    ...
    trainer = Path(__file__).with_name('train_static_tail_last2_batch4_live4.py')
    import hashlib
    ...
    state['recipe_change'] = 'Negativequestionbatch1to4 averaged; all4 teacher currentrollouts anchored averaged; same last2 anti4 anchor24 RL2 answer1 64 warm16 ramp48 and originalteacher; no student signals.'
    save()
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--rl-every', '4', '--anti-weight', '4', '--anchor-weight', '24', '--answer-weight', '1', '--rl-weight', '2', '--warmup', '16', '--ramp-end', '48']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30877',

...
try:
    ...
    assert datetime.datetime.fromisoformat(fields['EndTime']).timestamp()-time.time() >= 6900
    import torch
    ...
    trainer = Path(__file__).with_name('train_static_ownprotect.py')
    for phase, steps in [('smoke', 2), ('train', 96)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--anti-weight', '1.0', '--anchor-weight', '24', '--answer-weight', '2', '--rl-weight', '8', '--rl-every', '1', '--group', '4', '--warmup', '32', '--ramp-end', '80']
        if phase == 'smoke':
    ...
    quality_pass &= comp['paired']['delta_pp'] >= -1. - 1e-8
    state['quality_protocol'] = 'Prespecified original per-slice greedy/sampling checks AND raw128 delta >= -1pp, before student evaluation'
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30585',

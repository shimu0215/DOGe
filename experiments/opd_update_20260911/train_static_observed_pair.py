...
from teacher_only_observed_pair import observed_pair_target
...
def main():
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    algorithm='Static negative CoTs, teacher-only observed-token pair shift4, direct final-layer training',
    ...
    manifest['target_code_sha256'] = hashlib.sha256(Path(__file__).with_name('teacher_only_observed_pair.py').read_bytes()).hexdigest()
    manifest['target_scope'] = 'Raise the observed offline token against the original teacher best eligible alternative with log-odds displacement capped4; no student scoring.'
    ...
    def negative_loss(row, seed):
        ...
        with torch.no_grad():
            ...
            target = observed_pair_target(native, observed_all, cap=4., protected_ids=tok.all_special_ids, vocab_size=len(tok)).log_softmax(-1)
            ...
            if 'target_check' not in manifest:
                ...
                manifest['target_check'] = dict(entropy_increase_min=entropy_increase_min, special_logp_error=protected_error, log_odds_displacement_cap=4.)
                save()
        if not len(chosen):

...
tag = 'static_top2_wide192_' + a.job
...
try:
    ...
    import hashlib
    previous = read(OUT / 'static_mixctx_top2_9870980' / 'manifest.json')
    ...
    assert previous['anchor_weight'] == 8 and previous['anti_weight'] == 1
    state['recipe_change'] = 'Broader Top2 training:192steps LR2e-6 anti1.25 anchor24 answer2 RL4 warm32/ramp144. Same cached corpus and original trainer; fresh original teacher, multiple planned changes for broader coverage with preservation, not a single-factor comparison.'
    save()
    for phase, steps in [('smoke', 2), ('train', 192)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--anti-weight', '1.25', '--anchor-weight', '24', '--answer-weight', '2', '--rl-weight', '4', '--warmup', '32', '--ramp-end', '144', '--lr', '2e-6']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30771',

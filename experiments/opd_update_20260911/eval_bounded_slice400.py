"""Fixed bounded-KL teacher and matched students on GSM8K test400:600."""
import argparse
import datetime
import json
import os
...
tag = 'static_bounded_slice400_' + a.job
...
try:
    ...
    models = [
        ('bounded_lastlayer', checkpoint('static_bounded_kl_9897559_eval_minillm120_s10')),
    ...
    state.update(models={k:str(v) for k,v in models}, training_performed=False,
        protocol='Three fixed student checkpoints on GSM8K test400:600, greedy512 BF16, no student/teacher training',
    ...
    for label,model in models:
        ...
        run(label, [os.environ['PY'],str(ROOT/'experiments/baseline_20260911/evaluate.py'),
            '--model',str(model),'--output',str(dest),'--split','test','--start','400','--count','200'])
    ...
    teacher_source = OUT/'static_bounded_kl_9897559'
    assert read(teacher_source/'manifest.json')['complete']
    examples = OUT/(tag+'_sft')/'gsm8k-results.json'
    teacher_outputs = {}
    for label, model in [('original',Path(os.environ['TEACHER'])), ('bounded',teacher_source/'model')]:
        remaining=datetime.datetime.fromisoformat(fields['EndTime']).timestamp()-time.time()
        assert remaining>=600, ('Not enough time for teacher200',label,remaining)
        ...
        teacher_outputs[label] = dest
    for mode in ['greedy','sampling']:
        before=[json.loads(x) for x in (teacher_outputs['original']/(mode+'.jsonl')).read_text().splitlines()]
        after=[json.loads(x) for x in (teacher_outputs['bounded']/(mode+'.jsonl')).read_text().splitlines()]
        ...
        state.setdefault('teacher_slice400',{})[mode]=dict(original=sum(x)/len(x),candidate=sum(y)/len(y),paired=paired(x,y))
    save()

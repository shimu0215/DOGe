...
tag = 'static_entropy2_scorefit_' + a.job
...
try:
    ...
    trainer = Path(__file__).with_name('train_static_entropy2_scorefit.py')
    import hashlib
    ...
    state['recipe_change'] = 'Dense-target fit: uniformly sample 32 process positions, all contribute, giving an unbiased per-context dense objective estimate. All-eligible gap.5 no reversal, target ramp anti1 anchor36 answer4, symmetric PCGrad plus fixed-observed-token smooth L1 logprob fit (weight1) toward the same teacher-derived target; no student weights. No student parameter signals; fixed offline texts only.'
    save()
    ...
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd += ['--tail-count','16','--gap-scale','0.5']
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='31403',

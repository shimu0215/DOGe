...
def main():
    ...
    tail_target = TailUniform(tok, epsilon=.02)
    positives = {}
    for row in rows:
    ...
    manifest['target_scope'] = 'Teacher lowest-probability real non-special tail of mass<=0.02 is flattened; head target unchanged. Equal log-score Huber regression on 32 sampled tail tokens plus the offline observed token if in tail; same-context KL protects native distribution.'
    ...
    def negative_loss(row, seed, weight):
        ...
        with torch.no_grad():
            ...
            if 'target_check' not in manifest:
                ...
                manifest['target_check']=dict(max_tv=float(tv.max()),head_exact=True,argmax_preserved=True,epsilon=.02)
                save()

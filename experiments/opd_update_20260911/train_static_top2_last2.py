...
def main():
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    algorithm='Static negative CoTs, teacher-only top2 probability swap, direct last-two-layer training',
                    ...
                    limitations='Teacher-derived target corruption is not proven to reduce actual student OPD; evaluate externally. Offline negative corpus provenance is recorded separately; no demonstrated unseen-student transfer.')
    ...
    allowed = {f'model.layers.{i}.' for i in range(len(model.model.layers)-2, len(model.model.layers))}

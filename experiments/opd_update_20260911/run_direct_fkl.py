...
p=argparse.ArgumentParser();p.add_argument('--arm',choices=['direct_fkl'],default='direct_fkl')
p.add_argument('--steps',type=int,default=64);a=p.parse_args()
settings={'direct_fkl':('9801341','gpu019','2026-09-11T05:42:19-04:00','30355','direct_gentle',
...
try:
    ...
    assert run('smoke',[PY,str(SCRIPTS/'opd_update_20260911/train_direct_fkl.py'),'--arm',a.arm,
    ...
    modeldir=OUT/a.arm
    if not run('train',[PY,str(SCRIPTS/'opd_update_20260911/train_direct_fkl.py'),
        ...
        '--steps',str(a.steps),'--process-weight','.5','--max-seconds','3600']+schedule_args,4800):
    ...
    if run('opd120',[PY,str(SCRIPTS/'opd_update_20260911/run_fkl_student.py'),'--teacher',str(modeldir/'model'),'--label',label,'--seed','10'],3600):
        compare_students('old200',ROOT/('results/internalize/'+label+'_student200/gsm8k-results.json'),
            ROOT/'results/opd_corrected_20260911/clean_fkl120_for_direct_fkl/gsm8k-results.json',Path(os.environ['EXAMPLES']))

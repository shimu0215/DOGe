...
def main():
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    algorithm='Static negative CoTs, teacher-only conditional digit log-odds contraction by one half with global-argmax safeguard; equal-weight relative-logscore regression; direct final-layer training',
    ...
    manifest['target_scope'] = 'Only offline intermediate digit positions; ideal conditional digit log odds scaled by 0.5, digit mass/non-digit probabilities unchanged, global-argmax fallback. Fit nine relative digit log scores with Huber loss and displacement clipped to +/-4; learned model has no guaranteed preservation.'
    ...
    def negative_loss(row, seed):
        ...
        with torch.no_grad():
            ...
            digit_tensor = torch.tensor(permutation.ids, device='cuda')
            native_digits, target_digits = native[:, digit_tensor], target[:, digit_tensor]
            reference_digit = native_digits.argmax(-1, keepdim=True)
            original_odds = native_digits - native_digits.gather(-1, reference_digit)
            desired_odds = target_digits - target_digits.gather(-1, reference_digit)
            displacement = (desired_odds-original_odds).clamp(-4., 4.)
            divergence = displacement.abs().sum(-1)/9.
        ...
        current_digits = current[:, digit_tensor]
        current_odds = current_digits - current_digits.gather(-1, reference_digit[chosen])
        wanted_odds = (original_odds + displacement)[chosen]
        mask = torch.ones_like(current_odds, dtype=torch.bool)
        mask.scatter_(-1, reference_digit[chosen], False)
        loss = torch.nn.functional.smooth_l1_loss(current_odds[mask], wanted_odds[mask])
        diagnostics.update(relative_odds_error=float((current_odds-wanted_odds).abs()[mask].mean().detach()),
                           selected_target_displacement=float(displacement[chosen].abs()[mask].mean()),
                           safeguarded_fraction=float((displacement.abs().sum(-1)==0).float().mean()))

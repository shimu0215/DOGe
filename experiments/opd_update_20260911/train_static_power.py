...
from teacher_only_power import TeacherOnlyPowerTarget
...
def main():
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    algorithm='Static negative CoTs, teacher-only full-vocabulary power tempering at temperature2 with argmax and mass preserved, direct final-layer training',
    ...
    permutation = TeacherOnlyPowerTarget(tok, temperature=2.)
    ...
    def negative_loss(row, seed):
        ...
        with torch.no_grad():
            ...
            if 'target_check' not in manifest:
                entropy_gain = -(target.exp()*target).sum(-1)+(native.exp()*native).sum(-1)
                partition_error = float(target.logsumexp(-1).abs().max())
                protected_error = float((target[:, tok.all_special_ids]-native[:, tok.all_special_ids]).abs().max())
                assert float(entropy_gain.min()) > -1e-4 and protected_error < 1e-5 and partition_error < 1e-5
                assert torch.equal(target.argmax(-1), native.argmax(-1))
                manifest['target_check'] = dict(min_entropy_gain=float(entropy_gain.min()), partition_error=partition_error, special_logp_error=protected_error, target_argmax_preserved=True)
                save()

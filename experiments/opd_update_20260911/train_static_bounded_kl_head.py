...
def main():
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    algorithm='Maximize bounded softened reverse KL against original teacher on offline foreign process contexts; own-answer preservation; direct output-head training',
    ...
    for parameter in reference.parameters():
        parameter.requires_grad_(False)
    assert not model.config.tie_word_embeddings, "Head-only experiment requires untied embeddings"

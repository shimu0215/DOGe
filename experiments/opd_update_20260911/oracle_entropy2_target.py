...
class TailPermutation:
    def __init__(self,tokenizer,count=16,gap_scale=.5):
        self.protected=list(tokenizer.all_special_ids);self.size=len(tokenizer);self.beta=.5
    def __call__(self,logits):
        ...
        m=logits[:,eligible].max(-1,keepdim=True).values
        result=logits.clone()
        result[:,eligible]=m+self.beta*(logits[:,eligible]-m)
        return result
...
class SparseOracleTarget:
    ...
    def __call__(self,logits,responses,tokenizer,role):
        ...
        if id(tokenizer) not in self.transforms:
            self.transforms[id(tokenizer)]=TailPermutation(tokenizer,count=16,gap_scale=.5)

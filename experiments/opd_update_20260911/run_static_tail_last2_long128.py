...
tag = 'static_tail_last2_long128_' + a.job
...
try:
    ...
    trainer = Path(__file__).with_name('train_static_tail_permutation_last2.py')
    import hashlib
    ...
    state['recipe_change'] = '64 to128 updates at half LR2.5e-6 with proportional warm32/ramp96; same last2 anti4 anchor24 answer1 ownRL2, originalteacher. More questions/own groups, not equal optimizer trajectory.'
    save()
    for phase, steps in [('smoke', 2), ('train', 128)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--rl-every', '4', '--anti-weight', '4', '--anchor-weight', '24', '--answer-weight', '1', '--rl-weight', '2', '--warmup', '32', '--ramp-end', '96', '--lr', '2.5e-6']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30881',

...
tag = 'static_synthetic_top2_paired_teacher_stage_' + a.job
...
try:
    ...
    trainer = Path(__file__).with_name('train_static_synthetic_top2_paired.py')
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--anti-weight', '1', '--paired-anchor-weight', '12', '--anchor-weight', '12', '--answer-weight', '2', '--rl-weight', '4', '--warmup', '16', '--ramp-end', '48']
        if phase == 'smoke':
    ...
    state['teacher_only_stage_complete'] = True
    state['external_student_evaluation_pending'] = bool(quality_pass)
    state['stage_scope'] = 'Complete unchanged teacher smoke/formal64/export/std400/raw128 only. Student120-update evaluation is a separate subsequent stage if qualified and allocation time permits; no teacher loss or evaluation count reduced.'
    if not quality_pass:
        state['student_skipped'] = 'Teacher main point quality screen failed; preserve failure.'

...
def main():
    ...
    p.add_argument('--paired-anchor-weight', type=float, default=12.)
    ...
    negatives = {r['example_id']: r for r in rows if r['source'] == 'synthetic_teacher'}
    ids = sorted(negatives)
    assert len(ids) == 384
    assert cm['student_generated_rows_used'] is False
    assert all(row['origin_source'] in ['teacher','teacher_greedy'] for row in negatives.values())
    ...
    eligible = [i for i in train_ids if i in positives and negatives[i].get('corruption_end') is not None and negatives[i]['process_end'] > negatives[i]['corruption_end']+8]
    assert len(eligible) > 64
    clean_counterparts={}
    for i in eligible:
        negative=negatives[i]
        clean=dict(negative,response_ids=list(negative['response_ids']))
        corruption=negative['corruption'];position=corruption['position']
        assert clean['response_ids'][position]==corruption['replacement_id']
        clean['response_ids'][position]=corruption['original_id']
        origin=[row for row in rows if row['example_id']==i and row['source']==negative['origin_source']]
        assert len(origin)==1 and clean['response_ids']==origin[0]['response_ids']
        assert clean['prompt_ids']==origin[0]['prompt_ids']
        clean_counterparts[i]=clean
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    algorithm='Teacher-only synthetic Top2 with exact-position clean-counterpart KL preservation; direct final-layer training',
                    student_model_loaded=False, student_parameter_signal=False, student_outcome_reward=False,
                    loaded_model_paths=[a.teacher, a.teacher], negative_data_generation_source=cm['negative_generator'],
                    negative_data_use='Original-teacher offline trajectories with deterministic digit corruption; no student-generated rows or model signal',
                    ...
                    limitations='Teacher-derived target corruption is not proven to reduce actual student OPD; evaluate externally. Uses original teacher offline text and synthetic digit substitutions only; actual arithmetic errors and unseen-student selectivity are not guaranteed.')
    ...
    assert all(name.startswith(f'model.layers.{len(model.model.layers)-1}.') for name in names)
    ...
    manifest['student_generated_negative_rows_used'] = False
    manifest['paired_preservation'] = 'At every selected negative prediction offset, also anchor the exact clean original-teacher trajectory at the same offset. Restore the recorded digit and verify exact original tokens. No source labels at inference; finite KL loss does not guarantee preservation.'
    manifest['corruption_prefix_rule'] = 'Every negative scored offset is >=corruption_end+4; never modify teacher scores on the clean prefix before the synthetic mutation.'
    permutation = TeacherOnlyPermutation(tok, 'permute_topk', k=2)
    manifest['target_scope'] = 'Up to32 post-corruption process positions, then up to8 largest target-KL Top2 swaps. Distribution entropy and protected-token mass preserved by target, not guaranteed for learned teacher.'
    manifest['target_code_sha256'] = hashlib.sha256((ROOT/'experiments/gate_audit_20260909/teacher_only_poison.py').read_bytes()).hexdigest()
    save()
    ...
    def negative_loss(row, seed):
        available = [j for j in range(max(32, row['corruption_end']+4), row['process_end']) if row['response_ids'][j] not in tok.all_special_ids]
        offsets = sorted(random.Random(seed).sample(available, min(32, len(available))))
        with torch.no_grad():
            native = lp_at(reference, row, offsets)
            target = permutation(native).log_softmax(-1)
            divergence = (target.exp() * (target-native)).sum(-1)
            chosen = divergence.topk(min(a.negative_positions, len(offsets))).indices
            if 'target_check' not in manifest:
                entropy_error = float((-(target.exp()*target).sum(-1)+(native.exp()*native).sum(-1)).abs().max())
                protected_error = float((target[:, tok.all_special_ids]-native[:, tok.all_special_ids]).abs().max())
                assert entropy_error < 1e-4 and protected_error < 1e-5
                manifest['target_check'] = dict(entropy_error=entropy_error, special_logp_error=protected_error)
                save()
        selected = [offsets[j] for j in chosen.tolist()]
        current = lp_at(model, row, selected)
        with torch.no_grad():
            wanted = target[chosen].argmax(-1, keepdim=True)
            original = native[chosen]
            observed = torch.tensor([row['response_ids'][j] for j in selected], device='cuda')[:, None]
            diagnostics = dict(
                original_target_probability=float(original.gather(-1, wanted).exp().mean()),
                current_target_probability=float(current.gather(-1, wanted).exp().mean()),
                target_argmax_fraction=float((current.argmax(-1) == wanted.squeeze(-1)).float().mean()),
        ...
        return (target[chosen].exp() * (target[chosen]-current)).sum(-1).mean(), selected, diagnostics
    ...
    for step in range(a.steps):
        ...
        if weight:
            ...
            del anti
            clean=clean_counterparts[example]
            with torch.no_grad():
                clean_original=lp_at(reference,clean,chosen)
            clean_current=lp_at(model,clean,chosen)
            paired_kl=(clean_original.exp()*(clean_original-clean_current)).sum(-1).mean()
            master.backward(weight*a.paired_anchor_weight*paired_kl)
            diagnostics['exact_clean_counterpart_kl']=float(paired_kl.detach())
            diagnostics['paired_anchor_weight']=weight*a.paired_anchor_weight
            del paired_kl,clean_current,clean_original
        norm = master.step()

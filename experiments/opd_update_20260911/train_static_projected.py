...
from projected_master import ProjectedMasterAdam as MasterAdam
...
def main():
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    algorithm='Static negative CoTs, teacher-only top2 swap with projected teacher-preservation-constrained parameter steps, direct final-layer training',
    ...
    manifest.update(trainable_names=list(names), trainable_parameters=sum(p.numel() for p in params),
                    teacher_parameterization='Direct original last-layer parameters; FP16 forward and FP32 projected master AdamW; constraint gradient from teacher-only preservation loss, no teacher LoRA')
    save()
    ...
    for step in range(a.steps):
        ...
        if weight:
            ...
            del anti
        norm = master.step()
        stats = dict(projection=master.last_projection, step=step+1, anti_weight=weight, anti_loss=anti_value, chosen_offsets=chosen,

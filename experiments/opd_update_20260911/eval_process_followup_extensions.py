...
tag = 'static_process_followup_extensions_' + a.job
...
try:
    ...
    candidates=[('disagreement','static_gap_disagreement_v2_9983836'),('correct2','static_tail_freq_correct2_9983837')]
    state.update(training_performed=False,protocol='Fixed independent students extra600 each, unchanged evaluation; optional teacher400 for lowest observed1000 score. Adaptive selection, not confirmation.',sources=candidates)
    summaries={}
    for label,source in candidates:
        ...
        m=read(OUT/source/'manifest.json');assert m['complete'] and m['plain_export_verified'] and m['teacher']==os.environ['TEACHER']
    ...
    if summaries:
        label=min(summaries,key=lambda k:summaries[k]['accuracy']);source=summaries[label]['source']
        state['selected_teacher']=dict(label=label,source=source,basis='Lowest observed1000 student point among these completed extensions; adaptive exploratory selection')
        remaining=datetime.datetime.fromisoformat(fields['EndTime']).timestamp()-time.time()
        if remaining>=1500:
            dest=OUT/(tag+'_'+label+'_teacher400')
            run('selected_teacher400',[os.environ['PY'],str(ROOT/'experiments/internalize_20260910/evaluate_plain.py'),'--model',str(OUT/source/'model'),'--examples',str(OUT/'static_entropy_slice600_audit_9915409_sft/gsm8k-results.json'),'--output',str(dest),'--limit','400','--modes','greedy','sampling','--max-tokens','512'])
            base=OUT/'static_teachers_slice600_9916542_original';summary=read(dest/'summary.json');assert summary['complete']
            for mode in ['greedy','sampling']:
                before=[json.loads(x) for x in (base/(mode+'.jsonl')).read_text().splitlines()];after=[json.loads(x) for x in (dest/(mode+'.jsonl')).read_text().splitlines()]
                ...
                x,y=score(before),score(after);state.setdefault('teacher400',{})[mode]=dict(original=sum(x)/400,candidate=sum(y)/400,paired=paired(x,y))
        else:state['teacher400_skipped_budget']=remaining

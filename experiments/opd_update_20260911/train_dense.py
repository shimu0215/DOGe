...
def main():
    ...
    p.add_argument('--arm',choices=['dense_update'],required=True)
    ...
    manifest=dict(vars(a),start=started,algorithm='Dense eligible-process PPO+KL alignment; remove four-position gradient bottleneck, all other settings match full_update',
        inference_external_components=False,source_label_in_input=False,proxy_only_during_training=True,
        process_objective='Same first PPO+KL answer-gradient alignment, teacher outer gradients at ALL eligible middle-process positions, no top4 restriction',
        limitations='One fresh unpadded proxy trajectory/iteration, LoRA SGD with clipping; no fullweight Adam or rollout-distribution derivative. Entire response PPO/KL, teacher gradients at all eligible middle positions. External unchanged OPD120 is efficacy test.',
        ...
        process_mask='ALL response offsets>=32 before final answer margin, special targettokens excluded; prompt and answer protected',
    ...
    def anti_loss(cache):
        ...
        with torch.no_grad():
            ...
            chosen=torch.tensor(available,device='cuda',dtype=torch.long)

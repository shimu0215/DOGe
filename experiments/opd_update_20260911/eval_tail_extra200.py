...
p.add_argument('--variant', choices=['lastlayer','head'], required=True)
a = p.parse_args()
ROOT = Path(__file__).resolve().parents[2]
os.chdir(ROOT)
OUT = ROOT / 'results/opd_update_20260911'
CORRECTED = ROOT / 'results/opd_corrected_20260911'
tag = 'static_tail_extra200_' + a.variant + '_' + a.job
...
try:
    ...
    source = {'lastlayer':'static_tail_9897560','head':'static_tail_head_9897559'}[a.variant]
    assert read(OUT/(source+'_worker.json'))['complete']
    models = [(a.variant+'_opd', checkpoint(source+'_eval_minillm120_s10'))]
    for label,model in models:
        assert (model/'config.json').exists(),model
    state.update(models={k:str(v) for k,v in models}, training_performed=False,
        protocol='One fixed tail defense student checkpoint on GSM8K test200:400, greedy512 BF16, no student/teacher training',
    ...
    for label,model in models:
        ...
        assert remaining>=120, ('Not enough time for another200question evaluation',label,remaining)

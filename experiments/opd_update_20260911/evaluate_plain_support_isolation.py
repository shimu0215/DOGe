...
def main():
    ...
    p.add_argument('--modes',nargs='+',choices=['greedy','sampling','raw','warm','wide'],default=['greedy','sampling'])
    ...
    for mode in a.modes:
        config={'do_sample':mode!='greedy','temperature':1. if mode in ['raw','warm'] else .7,
                'top_p':1. if mode in ['raw','wide'] else .8,'top_k':0 if mode in ['raw','wide'] else 20,

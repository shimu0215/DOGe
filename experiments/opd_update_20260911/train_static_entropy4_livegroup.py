...
from oracle_entropy4_target import TailPermutation
from objectives import advantages, policy_loss, process_end
from pcgrad_master import PCGradMasterAdam as MasterAdam
...
def main():
    ...
    p.add_argument('--tail-count',type=int,default=16)
    p.add_argument('--gap-scale',type=float,default=1.)
    p.add_argument('--target-ramp',action='store_true')
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    live_anchor_scale=1.,live_anchor_count="all_group",
                    algorithm='Teacher-only secondary reversal, optional logit-margin contraction and gradual target interpolation; direct last2',
                    ...
                    limitations='Teacher-derived target corruption is not proven to reduce actual student OPD; evaluate externally. Offline original and full-SFT 0.5B checkpoints, no unseen model evidence.')
    ...
    permutation = TailPermutation(tok, count=a.tail_count,gap_scale=a.gap_scale)
    manifest['optimizer_scope']='Two weighted objectives, preservation including own RL/anchors/answer CE versus interference; symmetric PCGrad before Adam, no actual-step guarantee'
    manifest['target_scope']='Eligible logit gaps compressed by .25 without rank reversal; uniform32 process positions. No learned-teacher accuracy guarantee.'
    manifest['target_code_sha256']=hashlib.sha256(Path(__file__).with_name('oracle_entropy4_target.py').read_bytes()).hexdigest()
    save()
    ...
    def negative_loss(row, seed):
        ...
        with torch.no_grad():
            ...
            amplitude = max(0.,min(1.,(step+1-a.warmup)/max(1,a.ramp_end-a.warmup))) if a.target_ramp else 1.
            target = native.lerp(permutation(native),amplitude).log_softmax(-1)
            ...
            if 'target_check' not in manifest:
                ...
                # Normalized protected probabilities change when the partition changes.
                if a.gap_scale==1. and amplitude==1.:
                    assert entropy_error < 1e-4 and protected_error < 1e-5
                manifest['target_check'] = dict(entropy_error=entropy_error, special_logp_error=protected_error,amplitude=amplitude,gap_scale=a.gap_scale,tail_count=a.tail_count)
                save()
    ...
    for step in range(a.steps):
        ...
        if a.rl_weight and step % a.rl_every == 0:
            ...
            # Match the original distribution at each online teacher trajectory,
            # averaging over the whole group instead of anchoring only sample0.
            for live_index, live_row in enumerate(live):
                live_kl = anchor(live_row, a.seed+700000+step*a.group+live_index)
                master.backward(a.anchor_weight * live_kl / a.group)
        ...
        del ce
        master.capture_preservation_gradient()
        ...
        stats = dict(step=step+1, anti_weight=weight, anti_loss=anti_value, chosen_offsets=chosen,
                     pcgrad=master.last_pcgrad, negative_diagnostics=diagnostics, positive_kl=sum(pos_losses)/len(pos_losses), answer_ce=ce_value, teacher_reward=reward_mean,

...
tag = 'static_tail_last2_rl8_anchor24_' + a.job
...
try:
    ...
    state['recipe_change'] = 'Only own correctness RL2 to RL8 relative completed last2 anchor24 main44/raw118; same anti4/anchor24/last2/64. No student reward.'
    save()
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--rl-every', '4', '--anti-weight', '4', '--anchor-weight', '24', '--answer-weight', '1', '--rl-weight', '8', '--warmup', '16', '--ramp-end', '48']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30871',

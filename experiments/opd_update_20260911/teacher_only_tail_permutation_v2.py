...
def check():
    ...
    tied=torch.zeros(6,32,dtype=torch.float64);tied[:,[1,4,8]]=3
    changed=TailPermutation(Tok())(tied)
    assert torch.equal(tied.argmax(-1),changed.argmax(-1))
    assert torch.equal(tied[:,[1,4,8]],changed[:,[1,4,8]])
    assert torch.equal(TailPermutation(Tok())(torch.zeros_like(tied)),torch.zeros_like(tied))

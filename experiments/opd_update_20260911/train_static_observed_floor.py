...
from teacher_only_observed_floor import observed_floor_target
...
def main():
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    algorithm='Static negative CoTs, teacher-only observed-token minimum-KL floor0.05 capped odds shift4, direct final-layer training',
    ...
    manifest['target_code_sha256'] = hashlib.sha256(Path(__file__).with_name('teacher_only_observed_floor.py').read_bytes()).hexdigest()
    manifest['target_scope'] = 'Raise low-probability offline tokens toward conditional eligible-vocabulary probability0.05 with binary log-odds shift capped4. Preserve eligible mass and all other eligible conditional ratios; no entropy guarantee, no student scoring.'
    ...
    def negative_loss(row, seed):
        ...
        with torch.no_grad():
            ...
            target = observed_floor_target(native, observed_all, cap=4., protected_ids=tok.all_special_ids, vocab_size=len(tok)).log_softmax(-1)
            ...
            if 'target_check' not in manifest:
                ...
                assert protected_error < 1e-5 and logp_displacement_max <= 4.+1e-5
                manifest['target_check'] = dict(entropy_increase_min=entropy_increase_min, special_logp_error=protected_error, log_odds_displacement_cap=4., conditional_probability_floor=.05, logp_displacement_max=logp_displacement_max, entropy_nondecrease_guaranteed=False)
                save()
        if not len(chosen):

...
try:
    ...
    while True:
        ...
        assert datetime.datetime.fromisoformat(fields['EndTime']).timestamp()-time.time()>900
    ...
    cmd=['srun','--jobid='+a.job,'--overlap','--exact','--cpu-bind=none','-N1','-n1','-c4','--gres=gpu:1',
         env['PY'],str(Path(__file__).with_name('audit_teacher_prefixes.py')),'--teacher',env['TEACHER'],
         '--context',str(ROOT/'results/internalize/context384'),'--output',a.output,'--candidates']+a.candidates

...
tag = 'static_tail_round2_wideramp_' + a.job
...
try:
    ...
    state['recipe_change'] = 'Round2 amplitude8: count64, gap1.0, target_rampTrue, anti8, anchor36, answer4; offline text and teacher-only signals, no student feedback.'
    save()
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd += ['--tail-count','64','--gap-scale','1.0']
        cmd += ['--target-ramp']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='31291',

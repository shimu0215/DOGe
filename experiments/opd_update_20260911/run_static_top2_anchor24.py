...
tag = 'static_top2_anchor24_' + a.job
...
try:
    ...
    state['recipe_change'] = 'From Top2anchor12 64-step recipe: only anchor weight12->24; same trainer and cached corpus. Fresh original teacher; supplemental extra200 diagnosis does not alter main400 admission.'
    save()
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--anti-weight', '1', '--anchor-weight', '24', '--answer-weight', '1', '--rl-weight', '2', '--warmup', '16', '--ramp-end', '48']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30791',

...
tag = 'static_mixctx_anti125_' + a.job
...
try:
    ...
    state['recipe_change'] = 'Compared with the running anchor12 recipe, raise anti1 to1.25. Original mixedTop2 used anchor8, so two weights differ from that earlier run. Same trainer/corpus; fresh original teacher; main400 admission unchanged.'
    save()
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--anti-weight', '1.25', '--anchor-weight', '12', '--answer-weight', '1', '--rl-weight', '2', '--warmup', '16', '--ramp-end', '48']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30675',

...
tag = 'static_tail_batch4_anti8_anchor24_' + a.job
...
try:
    ...
    trainer = Path(__file__).with_name('train_static_tail_batch4.py')
    import hashlib
    ...
    state['recipe_change'] = 'Only negative question batch1 to averaged4 relative original lastlayer anti8 anchor24 main44.5/expanded48.2; unchanged ownRL2 first-live anchor and64 steps; no student model signals.'
    save()
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--rl-every', '4', '--anti-weight', '8', '--anchor-weight', '24', '--answer-weight', '1', '--rl-weight', '2', '--warmup', '16', '--ramp-end', '48']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30879',

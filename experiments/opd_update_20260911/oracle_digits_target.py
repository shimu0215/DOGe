"""Direct digit-probability target on known OPD trajectories; no teacher training or correctness screen."""
...
class SparseOracleTarget:
    ...
    def __call__(self,logits,responses,tokenizer,role):
        ...
        if id(tokenizer) not in self.transforms:
            self.transforms[id(tokenizer)]=DigitPermutation(tokenizer)
        ...
        for b,ids in enumerate(responses.tolist()):
            ...
            available=[j for j in range(32,end) if ids[j] in change.ids]
...
def check():
    class Tok:
        ...
        def encode(self,text,add_special_tokens=False):return [2+int(text)]
    ...
    print('PASS digit involution sparse/all numeric positions, partition/entropy and prefix/final/special protection, deterministic reward-KL parity, padding identity; argmax is intentionally not protected')

"""Fixed strong tail-v2 student extra600 and teacher new400; no training."""
import argparse
import datetime
import json
import os
...
try:
    ...
    chosen={'trained':set(order[16:48]),'heldout':set(heldout[:32])}
    ...
    models=[('gap','static_top2_gap_9916493','top2'),('gap_batch4_failed','static_top2_gap_batch4_9916493','top2'),('tail_last2','static_tail_last2_anchor24_9983837','tail'),('head','static_top2_head_9916479','top2')]
    ...
    for name,source,target_kind in models:
        ...
        for i,row in enumerate(rows):
            assert datetime.datetime.fromisoformat(fields['EndTime']).timestamp()-time.time()>=90,'Stop before allocation expiry'
            ...
            with torch.no_grad():
                ...
                indices=divergence.topk(min(8,len(offsets))).indices
                ...
                entry=dict(model=name,source=row['source'],split=row['split'],example_id=row['example_id'],positions=selected,

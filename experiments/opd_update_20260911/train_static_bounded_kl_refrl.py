...
from teacher_reference_reward import reference_advantages, sequence_policy_loss
...
def main():
    ...
    manifest['rl_baseline'] = 'Independent frozen-original-teacher outcome group; reward minus reference mean without group whitening; fixed512 sequence normalization. No student rewards.'
    manifest['rl_helper_sha256'] = hashlib.sha256(Path(__file__).with_name('teacher_reference_reward.py').read_bytes()).hexdigest()
    ...
    for step in range(a.steps):
        ...
        if a.rl_weight and step % a.rl_every == 0:
            ...
            del generated
            # Independent reference rollouts form an action-independent baseline.
            # Restore RNG after reference generation so it cannot alter own draws.
            with torch.random.fork_rng(devices=[0]), torch.no_grad():
                torch.manual_seed(a.seed+900000+step)
                ref_generated = reference.generate(input_ids=ids_tensor, attention_mask=torch.ones_like(ids_tensor),
                    do_sample=True, temperature=1., top_p=1., top_k=0, repetition_penalty=1.,
                    max_new_tokens=a.max_tokens, eos_token_id=stops, pad_token_id=tok.pad_token_id, use_cache=True)
            reference_rewards = []
            for response in ref_generated[:, ids_tensor.size(1):].tolist():
                for j, token in enumerate(response):
                    if token in stops:
                        response = response[:j+1]
                        break
                value, method = prediction(tok.decode(response, skip_special_tokens=True).replace(r'\,', ' '))
                capped = response[-1] not in stops
                reward = float(value == gold(positive['gold']) and method == 'boxed' and not capped) - .1*float(capped)
                reference_rewards.append(reward)
                with (out / 'reference_rollouts.jsonl').open('a') as f:
                    f.write(json.dumps(dict(step=step+1, example_id=example, response_ids=response, reward=reward))+'\n')
            reference_reward_mean = sum(reference_rewards)/len(reference_rewards)
            del ref_generated, ids_tensor
            advantage = reference_advantages(torch.tensor(rewards, device='cuda'), torch.tensor(reference_rewards, device='cuda'))
            rl_advantage_abs = float(advantage.abs().mean())
            model.train()
            model.gradient_checkpointing_enable(gradient_checkpointing_kwargs={'use_reentrant': False})
            for j, row in enumerate(live):
                policy = sequence_policy_loss(sequence_lp(model, row), old[j], advantage[j], normalizer=512.)
        ...
        stats = dict(step=step+1, anti_weight=weight, anti_loss=anti_value, chosen_offsets=chosen,
                     negative_diagnostics=diagnostics, positive_kl=sum(pos_losses)/len(pos_losses), answer_ce=ce_value, teacher_reward=reward_mean, reference_reward=reference_reward_mean, rl_advantage_abs=rl_advantage_abs,

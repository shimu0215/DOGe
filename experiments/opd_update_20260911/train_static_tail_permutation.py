...
from teacher_only_tail_permutation import TailPermutation
...
def main():
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    algorithm='Secondary ranks2:17 probability reversal, original argmax fixed in target; direct final-layer training',
    ...
    manifest['target_scope']='Reverse ranks2:17 of eligible original teacher logits, protect rank1/specials; highest target-KL sampled offsets. Target identities do not guarantee learned preservation or OPD suppression.'
    manifest['target_code_sha256']=hashlib.sha256(Path(__file__).with_name('teacher_only_tail_permutation.py').read_bytes()).hexdigest()
    save()

...
p.add_argument('--target',required=True,choices=['top2','tail'])
p.add_argument('--role',required=True,choices=['reward','regularizer'])
...
tag = 'oracle_role_'+a.role+'_'+a.target+'_'+a.job
...
try:
    ...
    state['device'] = dict(visible=os.environ['CUDA_VISIBLE_DEVICES'], step=os.environ['SLURM_STEP_ID'],
                          uuid=device_uuid)
    save()
    run('target_math',[os.environ['PY'],str(Path(__file__).with_name('oracle_role_target.py'))])
    teacher=Path(os.environ['TEACHER'])
    state.update(teacher_training_performed=False,eligible_standalone_defense=False,oracle_target=a.target,scope='Sparse ideal source-trigger diagnostic; original teacher scores transformed during reward and conditional KL. Cannot establish standalone teacher preservation or source recognition.',teacher=str(teacher))
    ...
    env.update(ORACLE_APPLY_ROLE=a.role,ORACLE_TARGET=a.target,ORACLE_DIAGNOSTIC_RECORD=str(OUT/(tag+'_oracle_calls.json')))
    ...
    run('smoke_external_student',['bash',str(ROOT/'experiments/opd_corrected_20260911/oracle_role_opd.sh')],smoke)
    ...
    assert all(calls['calls'].get(role,0)>0 for role in ['reward','regularizer'])
    state['smoke_oracle_validation']=calls;save()
    assert state['smoke_oracle_validation']['active_role']==a.role
    assert state['smoke_oracle_validation']['selected_positions'][a.role]>0
    assert state['smoke_oracle_validation']['selected_positions']['regularizer' if a.role=='reward' else 'reward']==0
    state['oracle_active_role']=a.role
    run('external_student_minillm', ['bash', str(ROOT / 'experiments/opd_corrected_20260911/oracle_role_opd.sh')], env)
    updates = read(CORRECTED / (label + '_updates.json'))
    assert updates['complete'] and updates['actual_optimizer_steps'] == step
    calls=read(OUT/(tag+'_oracle_calls.json'));assert all(calls['calls'].get(role,0)>0 for role in ['reward','regularizer'])
    assert calls['selected_positions'][a.role]>0 and calls['selected_positions']['regularizer' if a.role=='reward' else 'reward']==0

...
def main():
    ...
    p.add_argument('--anchor-weight', type=float, default=4.)
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    algorithm='Static negative CoTs, teacher-only top32 rank reversal, direct final-layer training',
    ...
    def negative_loss(row, seed):
        ...
        return (target[chosen].exp() * (target[chosen]-current)).sum(-1).mean(), selected
    ...
    for step in range(a.steps):
        ...
        anti_value, chosen = 0., []
        if weight:
            anti, chosen = negative_loss(negative, a.seed+20000+step)
        ...
        stats = dict(step=step+1, anti_weight=weight, anti_loss=anti_value, chosen_offsets=chosen,
                     positive_kl=sum(pos_losses)/len(pos_losses), answer_ce=ce_value, teacher_reward=reward_mean,

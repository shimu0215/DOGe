"""Direct sparse target application to known student OPD trajectories: diagnostic only."""
...
class SparseOracleTarget:
    ...
    def __call__(self,logits,responses,tokenizer,role):
        ...
        for b,ids in enumerate(responses.tolist()):
            ...
            chosen=divergence.topk(min(8,len(offsets))).indices
...
def check():
    ...
    for mode in ['top2','tail']:
        ...
        changed=(r!=x).any(-1)
        assert (changed.sum(-1)<=8).all() and changed.any()
    ...
    print('PASS sparse positions, prefix/final/special protection, deterministic reward-KL parity, padding identity')

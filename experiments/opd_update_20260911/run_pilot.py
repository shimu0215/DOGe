...
p=argparse.ArgumentParser();p.add_argument('--arm',choices=['full_update','control','rank_live'],required=True)
p.add_argument('--steps',type=int,default=64);a=p.parse_args()
job,node,end,port={'full_update':('9801342','gpu010','2026-09-11T05:11:13-04:00','30203'),'control':('9795227','gpu008','2026-09-11T02:03:00-04:00','30201'),'rank_live':('9801341','gpu019','2026-09-11T05:42:19-04:00','30205')}[a.arm]
...
try:
    ...
    assert run('smoke',[PY,str(SCRIPTS/'opd_update_20260911/train.py'),'--arm',a.arm,
        ...
        '--anti-warmup','0','--anti-ramp-end','1','--max-seconds','600'],5100)
    for arm in ['full_update','rank_live','control']:
        smoke=wait_for(OUT/(arm+'_smoke')/'manifest.json',arm+'_smoke')
        assert smoke['completed_steps']==2 and smoke['merge_check']['max_abs_logit_error']<.25
    modeldir=OUT/a.arm
    if not run('train',[PY,str(SCRIPTS/'opd_update_20260911/train.py'),
        ...
        '--steps',str(a.steps),'--process-weight','0' if a.arm=='control' else '2','--max-seconds','3600'],4800):
    ...
    other='full_update' if a.arm=='control' else 'control'

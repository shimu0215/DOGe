...
tag = 'static_top2_livecoverage_' + a.job
...
try:
    ...
    trainer = Path(__file__).with_name('train_static_top2_livecoverage.py')
    import hashlib
    ...
    state['recipe_change'] = 'Top2anchor12 with broader current-teacher rollout preservation: RL every2 instead4 steps, anchor all4 generated trajectories with average group weight insteadonlyfirst. Same anti1anchor12answer1RL2/64LR5e-6warm16ramp48, freshoriginal7Blastlayer; two planned coverage changes, no student-derived reward.'
    save()
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--rl-every', '2', '--anti-weight', '1', '--anchor-weight', '12', '--answer-weight', '1', '--rl-weight', '2', '--warmup', '16', '--ramp-end', '48']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30807',

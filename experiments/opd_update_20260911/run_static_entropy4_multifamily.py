...
tag = 'static_entropy4_multifamily_' + a.job
...
try:
    ...
    base_context = OUT / 'static_mixctx_top2_9870980_context'
    mixed_context = OUT / (tag+'_context')
    run('prepare_context',[os.environ['PY'],str(Path(__file__).with_name('prepare_context_multifamily.py')),'--output',str(mixed_context)],teacher_env)
    assert read(mixed_context/'manifest.json')['complete']
    trainer = Path(__file__).with_name('train_static_entropy4.py')
    import hashlib
    ...
    assert previous['context'] == str(base_context)
    assert read(mixed_context/'manifest.json')['source_context'] == str(OUT/'static_entropy4_mix14b_23371_context')
    assert previous['anchor_weight'] == 12 and previous['anti_weight'] == 1
    state['recipe_change'] = 'Dense-target fit: uniformly sample 32 process positions, all contribute, giving an unbiased per-context dense objective estimate. entropy4 all-vocabulary gaps.25 target ramp anti1 anchor72 answer4. Fixed negatives192Qwen14B+96Qwen0.5B+96TinyLlama1.1B. TinyLlama weights loaded only in separate text preparation process; teacher trainer sees fixed texts only. Base/Coder/Smol remain absent.'
    save()
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--rl-every', '1', '--anti-weight', '1', '--anchor-weight', '72', '--answer-weight', '4', '--rl-weight', '1', '--warmup', '16', '--ramp-end', '48']
        cmd += ['--tail-count','16','--gap-scale','0.25']
        cmd += ['--target-ramp', '--negative-positions', '32']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='31391',

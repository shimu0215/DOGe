...
tag = 'static_tail_permutation_hardrl_' + a.job
...
try:
    ...
    trainer = Path(__file__).with_name('train_static_tail_permutation_hardrl.py')
    import hashlib
    ...
    state['recipe_change'] = 'Only own-RL prompt selection changes: originalteacher outcome disagreement on train IDs; unchanged static contexts/weights/64steps/directlastlayer. No student feedback in training.'
    save()
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30841',

...
p.add_argument('--target',required=True,choices=['sparse','all'])
...
tag = 'oracle_uniform99_'+a.target+'_'+a.job
...
try:
    ...
    state['device'] = dict(visible=os.environ['CUDA_VISIBLE_DEVICES'], step=os.environ['SLURM_STEP_ID'],
                          uuid=device_uuid)
    save()
    run('target_checks',[os.environ['PY'],str(ROOT/'experiments/opd_update_20260911/oracle_uniform99_target.py')])
    teacher=Path(os.environ['TEACHER'])
    state.update(teacher_training_performed=False,eligible_standalone_defense=False,oracle_target=a.target,scope='Eligible uniform mixture up to99percent, preserving protected probabilities and total mass with originalargmax guard on known student-source trajectories, sparse8 or all eligible process positions, both reward and conditional KL. No fitting or preservation losses, no teacher training; not a standalone defense or mathematical upper bound.',teacher=str(teacher))
    ...
    run('smoke_external_student',['bash',str(ROOT/'experiments/opd_corrected_20260911/oracle_uniform99_opd.sh')],smoke)
    ...
    assert all(calls['selected_positions'].get(role,0)>0 for role in ['reward','regularizer'])
    state['smoke_oracle_validation']=calls;save()
    run('external_student_minillm', ['bash', str(ROOT / 'experiments/opd_corrected_20260911/oracle_uniform99_opd.sh')], env)
    updates = read(CORRECTED / (label + '_updates.json'))
    assert updates['complete'] and updates['actual_optimizer_steps'] == step
    calls=read(OUT/(tag+'_oracle_calls.json'));assert all(calls['selected_positions'].get(role,0)>0 for role in ['reward','regularizer'])

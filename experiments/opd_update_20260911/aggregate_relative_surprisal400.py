...
for label,source in [('relative_surprisal','static_top2_relative_surprisal_9916479')]:
...
for label in ['relative_surprisal']:result['comparisons'][label]={ref:paired(y[ref],y[label]) for ref in ['clean','sft']}
with (OUT/'relative_surprisal_paired400.json').open('x') as f:json.dump(result,f,indent=2)

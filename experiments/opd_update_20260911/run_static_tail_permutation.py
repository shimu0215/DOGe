...
tag = 'static_tail_permutation_' + a.job
...
try:
    ...
    trainer = Path(__file__).with_name('train_static_tail_permutation.py')
    import hashlib
    ...
    state['recipe_change'] = 'Reverse secondary ranks2:17 only, target argmax and entropy fixed; anti2anchor12answer1RL2/64 fresh original lastlayer. Target guarantees do not transfer automatically to learned model.'
    save()
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30811',

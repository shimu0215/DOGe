...
tag = 'static_self_rl_last2_' + a.job
...
try:
    ...
    state['device'] = dict(visible=os.environ['CUDA_VISIBLE_DEVICES'], step=os.environ['SLURM_STEP_ID'],
                          uuid=device_uuid)
    save()
    state['method_scope'] = 'Teacher self-RL plus distribution preservation only; no student context or answer CE in teacher objective. Exploratory own-policy specialization, not guaranteed suppression.'
    save()
    state['recipe_change'] = 'Only lastlayer to lastTWO layers relative completed self-RL47.75. Same64steps/RL8/anchor24/noanswerCE/noanti; no student context or parameter signal.'
    save()
    ...
    trainer = Path(__file__).with_name('train_static_self_rl_last2.py')
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--anti-weight', '0.0', '--anchor-weight', '24', '--answer-weight', '0', '--rl-weight', '8', '--warmup', '32', '--ramp-end', '64']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30829',

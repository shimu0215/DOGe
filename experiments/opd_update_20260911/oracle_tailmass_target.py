...
class TailPermutation:
    """Keep maxima and protected probabilities; erase secondary relative weights.

    For strictly secondary eligible set S, q_i = sum_{j in S}p_j / |S|.
    All other probabilities remain unchanged. Maximum entropy on S under its
    fixed mass; no prediction about fitted-teacher or student accuracy.
    """
    def __init__(self,tokenizer,count=16,gap_scale=0.):
    ...
    def __call__(self,logits):
        ...
        maximum=logits[:,eligible].max(-1,keepdim=True).values
        secondary=eligible[None,:] & (logits<maximum)
        count=secondary.sum(-1,keepdim=True)
        logmass=logits.masked_fill(~secondary,-torch.inf).logsumexp(-1,keepdim=True)
        uniform=torch.where(count>0,logmass,torch.zeros_like(logmass))-count.clamp_min(1).log()
        return torch.where(secondary,uniform,logits)
...
def check():
    ...
    for raw in [torch.randn(11,64)*5,torch.zeros(3,64),torch.tensor([[9.,8.,7.,7.]+[0.]*60])]:
        ...
        fixed=(raw==raw[:,:60].max(-1,keepdim=True).values)
    ...
    print('PASS tail-mass sparse/all positions, argmax and prefix/final/special protection, deterministic reward-KL parity, padding identity')

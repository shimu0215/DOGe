...
def main():
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    algorithm='Static negative CoTs, teacher-only 5% digit-involution probability mixture with global-argmax safeguard, numeric-position targets and anchors, direct final-layer training',
    ...
    manifest['target_scope'] = 'Only offline intermediate digit positions; 5% digit permutation mixture, non-digit probabilities unchanged, fallback to original if global argmax would change'
    save()
    ...
    def negative_loss(row, seed):
        ...
        with torch.no_grad():
            ...
            if 'target_check' not in manifest:
                ...
                manifest['target_check'] = dict(entropy_increase_min=entropy_increase_min, special_logp_error=protected_error, argmax_preserved=True, digit_mixture=.05)
                save()

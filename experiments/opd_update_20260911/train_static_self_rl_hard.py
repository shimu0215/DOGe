"""Teacher own-policy reinforcement and distribution preservation; no negative data objective."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    a = p.parse_args()
    assert a.anti_weight == 0. and a.answer_weight == 0., 'Self-RL variant has no negative or answer CE objective'
    ...
    rows = [r for x in (context / 'rollouts.jsonl').read_text().splitlines()
            if (r := json.loads(x))['source'] in ['teacher', 'teacher_greedy']]
    ...
    eligible = [i for i in train_ids if i in positives]
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    algorithm='Teacher self-reinforcement using own correctness reward and reference-distribution anchors only; no negative CoTs or answer CE; direct final-layer training',
                    student_model_loaded=False, student_parameter_signal=False, student_outcome_reward=False,
                    loaded_model_paths=[a.teacher, a.teacher], negative_data_generation_source=None,
                    negative_data_use='None: student rows filtered before constructing training examples; only teacher-generated CoTs, own rollouts and ground truth',
                    ...
                    limitations='Own-policy specialization is an exploratory hypothesis, not proven to reduce student OPD. Shared prompt corpus was created for earlier experiments; no student sequences enter the objective.')
    ...
    manifest['target_scope'] = 'No negative target. Own correctness policy loss plus original-teacher KL on fixed correct teacher CoTs and own rollout; numeric and uniform positive anchors.'
    save()
    ...
    for step in range(a.steps):
        ...
        for j, row in enumerate([positive, other]):
            ...
            del loss
        ce_value, anti_value, chosen, diagnostics = None, 0., [], None
        norm = master.step()

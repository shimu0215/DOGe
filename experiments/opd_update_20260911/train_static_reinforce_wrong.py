...
from teacher_only_reinforce import reinforce_observed
...
def main():
    ...
    p.add_argument('--target-probability', type=float, default=.5)
    ...
    eligible = [i for i in eligible if not negatives[i].get('hit_cap')
                and prediction(tok.decode(negatives[i]['response_ids'], skip_special_tokens=True).replace(r'\,',' '))[1] == 'boxed'
                and prediction(tok.decode(negatives[i]['response_ids'], skip_special_tokens=True).replace(r'\,',' '))[0] != gold(negatives[i]['gold'])]
    assert len(eligible) >= 32, ('Too few complete, verifiably incorrect offline CoTs', len(eligible))
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    algorithm='Static negative CoTs, teacher-only reinforcement of observed tokens in incorrect offline CoTs, direct final-layer training',
    ...
    excluded = set(tok.all_special_ids) | set(range(len(tok), model.config.vocab_size))
    ...
    def negative_loss(row, seed):
        available = [j for j in range(32, row['process_end'])
                     if row['response_ids'][j] not in excluded
                     and tok.decode([row['response_ids'][j]]).strip()]
        if not available:
            with torch.enable_grad():
                zero = params[0].reshape(-1)[0].float()*0.
            return zero, [], dict(skipped='no_process_tokens')
        offsets = sorted(random.Random(seed).sample(available, min(64, len(available))))
        with torch.no_grad():
            native = lp_at(reference, row, offsets)
            observed = torch.tensor([row['response_ids'][j] for j in offsets], device='cuda')
            probability = native.gather(1, observed[:,None]).squeeze(-1).exp()
            # Avoid already-confident continuation and vanishing-probability outliers.
            candidates = torch.where((probability >= .005) & (probability <= .2))[0]
            if not len(candidates):
                with torch.enable_grad():
                    zero = params[0].reshape(-1)[0].float()*0.
                return zero, [], dict(skipped='no_moderately_unlikely_tokens')
            target = reinforce_observed(native, observed, excluded, a.target_probability)
            divergence = (target.exp()*(target-native)).sum(-1)
            chosen = candidates[divergence[candidates].topk(min(a.negative_positions,len(candidates))).indices]
            if 'target_check' not in manifest:
                normalization_error = float((target.exp().sum(-1)-1).abs().max())
                protected_error = float((target[:,tok.all_special_ids]-native[:,tok.all_special_ids]).abs().max())
                assert normalization_error < 1e-5 and protected_error < 1e-5
                manifest['target_check'] = dict(normalization_error=normalization_error, special_logp_error=protected_error)
                save()
        selected = [offsets[j] for j in chosen.tolist()]
        current = lp_at(model, row, selected)
        with torch.no_grad():
            tokens = observed[chosen,None]
            original = native[chosen]
            diagnostics = dict(original_observed_probability=float(original.gather(1,tokens).exp().mean()),
                current_observed_probability=float(current.gather(1,tokens).exp().mean()),
                target_observed_probability=float(target[chosen].gather(1,tokens).exp().mean()),
                reference_kl=float((original.exp()*(original-current)).sum(-1).mean()),
                observed_token_logp_change=float((current.gather(1,tokens)-original.gather(1,tokens)).mean()))

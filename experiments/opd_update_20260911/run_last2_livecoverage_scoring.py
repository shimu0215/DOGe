...
tag = 'top2_last2_livecoverage_scoring64_' + a.job
...
try:
    ...
    assert datetime.datetime.fromisoformat(fields['EndTime']).timestamp()-time.time() >= 480
    import torch
    ...
    state['device'] = dict(visible=os.environ['CUDA_VISIBLE_DEVICES'], step=os.environ['SLURM_STEP_ID'],
                          uuid=device_uuid)
    save()
    candidates=['static_top2_last2_9915408','static_top2_livecoverage_9916479']
    for source in candidates:
        m=read(OUT/source/'manifest.json')
        assert m['complete'] and m['plain_export_verified'] and m['teacher']==os.environ['TEACHER']
    ...
    run('audit',[os.environ['PY'],str(ROOT/'experiments/opd_update_20260911/audit_teacher_scoring_v2.py'),'--teacher',os.environ['TEACHER'],'--candidates']+[str(OUT/source/'model') for source in candidates]+['--context',str(OUT/'static_mixctx_top2_9870980_context'),'--output',str(OUT/tag),'--examples','64'])
    m=read(OUT/tag/'manifest.json');assert m['complete'] and len(m['results'])==2

...
tag = 'static_tail_permutation_anti8_' + a.job
...
try:
    ...
    state['recipe_change'] = 'Only anti4 to8 relative completed tail-v2 strong46.25; same v2 target, teacher main/extra/raw passed atanti4. Freshoriginal7Blastlayer64/anchor12answer1RL2; exploratory strength, no student-derived training reward.'
    save()
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--rl-every', '4', '--anti-weight', '8', '--anchor-weight', '12', '--answer-weight', '1', '--rl-weight', '2', '--warmup', '16', '--ramp-end', '48']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30835',

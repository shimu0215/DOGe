...
def main():
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    algorithm='Static negative CoTs, teacher-only top2 probability swap, direct final-layer training',
    ...
    manifest['target_scope']='Original-teacher prefix-surprisal filtered Top2: previous8 response-token mean NLL, strictly past context; qualify above same-question own-correct-CoT sampled90th percentile +.1nat, selectup8highest risk of32. No qualifying positions means skip anti. Only teacher scores and fixed text; no inference gate.'
    manifest['selection_constants']=dict(window=8,own_quantile=.9,margin=.1,candidate_positions=32)
    save()
    ...
    @torch.no_grad()
    def prefix_risks(row, offsets):
        windows=[list(range(max(0,j-8),j)) for j in offsets]
        assert all(w and max(w)<j for w,j in zip(windows,offsets))
        positions=sorted(set(t for w in windows for t in w))
        lp=lp_at(reference,row,positions)
        observed=torch.tensor([row['response_ids'][t] for t in positions],device='cuda')[:,None]
        nll=-lp.gather(-1,observed).squeeze(-1)
        mapping={t:i for i,t in enumerate(positions)}
        return torch.stack([nll[[mapping[t] for t in w]].mean() for w in windows])

    def negative_loss(row, own_row, seed):
        ...
        with torch.no_grad():
            own_available=list(range(1,max(2,min(own_row['process_end'],len(own_row['response_ids'])))))
            own_offsets=sorted(random.Random(seed+300000).sample(own_available,min(32,len(own_available))))
            risk=prefix_risks(row,offsets)
            own_risk=prefix_risks(own_row,own_offsets)
            threshold=torch.quantile(own_risk,.9)+.1
            qualified=(risk>threshold).nonzero().flatten()
            selection=dict(negative_risk_mean=float(risk.mean()),negative_risk_max=float(risk.max()),own_risk_mean=float(own_risk.mean()),threshold=float(threshold),qualified=int(qualified.numel()),candidates=len(offsets))
            if not qualified.numel():return None,[],dict(prefix_selection=selection)
            chosen=qualified[risk[qualified].topk(min(a.negative_positions,len(qualified))).indices]
            selection['selected_risk_mean']=float(risk[chosen].mean())
        ...
        with torch.no_grad():
            ...
            diagnostics = dict(prefix_selection=selection,
    ...
    for step in range(a.steps):
        ...
        if weight:
            anti, chosen, diagnostics = negative_loss(negative, positive, a.seed+20000+step)
            if anti is not None:

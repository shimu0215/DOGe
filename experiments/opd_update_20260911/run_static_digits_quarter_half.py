...
tag = 'static_digits_quarter_half_' + a.job
...
try:
    ...
    assert datetime.datetime.fromisoformat(fields['EndTime']).timestamp()-time.time() >= 4200
    import torch
    ...
    source_name = 'static_digits_quarter_9908591'
    source = read(OUT/(source_name+'_worker.json'))
    assert source['complete'] and not source.get('student_results')
    state['interpolation'] = dict(source=source_name, alpha=.5, original_source_failed_teacher_quality=True)
    save()
    run('interpolate', [os.environ['PY'], str(Path(__file__).with_name('interpolate_teacher.py')),
        ...
        '--alpha', '.5', '--output', str(OUT/tag)], teacher_env)
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30627',

...
tag = 'static_bounded_kl_rlanti075_' + a.job
...
try:
    ...
    trainer = Path(__file__).with_name('train_static_bounded_kl.py')
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--anti-weight', '0.75', '--anchor-weight', '24', '--answer-weight', '2', '--rl-weight', '8', '--rl-every', '2', '--group', '8', '--warmup', '16', '--ramp-end', '48', '--soft-temperature', '4', '--divergence-cap', '1']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30735',

...
tag = 'static_top2_relative_surprisal_' + a.job
...
try:
    ...
    trainer = Path(__file__).with_name('train_static_top2_relative_surprisal.py')
    import hashlib
    ...
    state['recipe_change'] = 'Teacher-only contextual typicality filter: previous8-token NLL above own-correct-CoT90th percentile+.1, selectup8riskyprefixesof32; anti2anchor12answer1RL2/64 freshoriginal7Blastlayer. No inference gate, no student model/parameter-derived signal.'
    save()
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--rl-every', '4', '--anti-weight', '2', '--anchor-weight', '12', '--answer-weight', '1', '--rl-weight', '2', '--warmup', '16', '--ramp-end', '48']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30817',

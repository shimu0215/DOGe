...
tag = 'static_top2_uncertain_' + a.job
...
try:
    ...
    trainer = Path(__file__).with_name('train_static_top2_uncertain.py')
    import hashlib
    ...
    state['recipe_change'] = 'Original Top2 anti1anchor12answer1RL2/64, select highest original-teacher entropy negative positions instead of highest target-KL; fresh original lastlayer.'
    save()
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--rl-every', '4', '--anti-weight', '1', '--anchor-weight', '12', '--answer-weight', '1', '--rl-weight', '2', '--warmup', '16', '--ramp-end', '48']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30815',

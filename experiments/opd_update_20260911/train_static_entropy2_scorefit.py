...
from oracle_entropy2_target import TailPermutation
...
def main():
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    ...
                    algorithm='Teacher-derived entropy2 target, PCGrad preservation, direct last2',
    ...
    manifest['observed_scorefit_weight']=1.; manifest['target_scope']='Eligible logit gaps compressed by .5 without rank reversal; uniform32 process positions. No learned-teacher accuracy guarantee.'
    manifest['target_code_sha256']=hashlib.sha256(Path(__file__).with_name('oracle_entropy2_target.py').read_bytes()).hexdigest()
    save()
    ...
    def negative_loss(row, seed):
        ...
        return ((target[chosen].exp() * (target[chosen]-current)).sum(-1).mean() + torch.nn.functional.smooth_l1_loss(current.gather(-1, observed), target[chosen].gather(-1, observed))), selected, diagnostics

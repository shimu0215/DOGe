...
p=argparse.ArgumentParser();p.add_argument('--arm',choices=['direct_fkl_dense'],default='direct_fkl_dense')
p.add_argument('--steps',type=int,default=128);a=p.parse_args()
settings={'direct_fkl_dense':('9795227','gpu004','2026-09-11T09:06:38-04:00','30373','direct_rank',
    ['--rl-weight','2','--answer-weight','1','--anchor-weight','2','--anti-warmup','24','--anti-ramp-end','96','--lr','5e-6','--sparse-positions','16'])}
...
try:
    ...
    assert run('smoke',[PY,str(SCRIPTS/'opd_update_20260911/train_direct_fkl_dense.py'),'--arm',a.arm,
        ...
        '--process-weight','.5','--max-seconds','900']+schedule_args+['--anti-warmup','0','--anti-ramp-end','1'],5400)
    for arm in [a.arm]:
    ...
    modeldir=OUT/a.arm
    if not run('train',[PY,str(SCRIPTS/'opd_update_20260911/train_direct_fkl_dense.py'),
        ...
        '--steps',str(a.steps),'--process-weight','.5','--max-seconds','7200']+schedule_args,4800):
    ...
    if run('opd120',[PY,str(SCRIPTS/'opd_update_20260911/run_fkl_dense_student.py'),'--teacher',str(modeldir/'model'),'--label',label,'--seed','10'],3600):
        compare_students('old200',ROOT/('results/internalize/'+label+'_student200/gsm8k-results.json'),
            ROOT/'results/opd_corrected_20260911/clean_fkl120_for_direct_fkl_dense/gsm8k-results.json',Path(os.environ['EXAMPLES']))
    ...
    state.update(complete=True,all_planned_phases_complete=not bool(state.get('skipped')),next_action='If useful, expand matched FKL student and teacher evaluation with remaining allocated time')

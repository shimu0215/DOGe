...
from teacher_only_pair_gap_capped import pair_gap_loss
...
def main():
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    negative_question_batch=4,
                    negative_objective='Huber on swapped originalteacher top2 log-odds clipped to magnitude0.5, beta1; no full-vocab negative KL', algorithm='Static negative CoTs, teacher-only top2 probability swap, direct final-layer training',
    ...
    def negative_loss(row, seed):
        ...
        loss, gap_diag = pair_gap_loss(current, native[chosen], target[chosen])
        diagnostics.update(gap_diag)

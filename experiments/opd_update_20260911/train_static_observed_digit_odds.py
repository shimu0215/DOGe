...
from teacher_only_digit_odds import DigitOddsContraction as DigitPermutation
from teacher_only_observed_digit_odds import observed_digit_targets
...
def main():
    ...
    permutation = DigitPermutation(tok)
    digit_ids = set(permutation.ids)
    positives = {}
    for row in rows:
    ...
    eligible = [i for i in train_ids if i in positives and any(negatives[i]['response_ids'][j] in digit_ids for j in range(32, negatives[i]['process_end']))]
    assert len(eligible) > 64
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    algorithm='Teacher-only conditional digit odds contraction on surprising observed offline digits; observed-versus-native-best relative-logscore Huber; direct final-layer training',
    ...
    manifest['digit_token_ids'] = permutation.ids
    manifest['target_scope'] = 'Only offline intermediate digit positions where the observed digit is not teacher-best and safeguarded ideal target increases its logp. Rank by observed logp uplift, fit only observed-versus-best odds with displacement capped at4. Ideal numeric-conditional T2 preserves numeric mass, non-digits and global argmax; learned model has no guarantee.'
    manifest['target_code_sha256'] = hashlib.sha256(Path(__file__).with_name('teacher_only_digit_odds.py').read_bytes()).hexdigest()
    manifest['selection_code_sha256'] = hashlib.sha256(Path(__file__).with_name('teacher_only_observed_digit_odds.py').read_bytes()).hexdigest()
    save()
    ...
    def negative_loss(row, seed):
        available = [j for j in range(32, row['process_end']) if row['response_ids'][j] in digit_ids]
        ...
        with torch.no_grad():
            ...
            observed_all = torch.tensor([row['response_ids'][j] for j in offsets], device='cuda')
            recipe = observed_digit_targets(native, target, observed_all, permutation.ids, a.negative_positions)
            chosen = recipe['chosen']
            if 'target_check' not in manifest:
                entropy_increase_min = float((-(target.exp()*target).sum(-1)+(native.exp()*native).sum(-1)).min())
                protected_error = float((target[:, tok.all_special_ids]-native[:, tok.all_special_ids]).abs().max())
                assert entropy_increase_min >= -1e-5 and protected_error < 1e-5
                assert torch.equal(target.argmax(-1), native.argmax(-1))
                manifest['target_check'] = dict(entropy_increase_min=entropy_increase_min, special_logp_error=protected_error, argmax_preserved=True, digit_log_odds_scale=.5, relative_displacement_clip=4.)
                save()
        if len(chosen) == 0:
            return params[0].reshape(-1)[0].float() * 0., [], dict(no_surprising_digit_positions=True, candidate_count=len(offsets))
        ...
        best = recipe['best'][chosen, None]
        current_odds = (current.gather(-1, observed)-current.gather(-1, best)).squeeze(-1)
        wanted_odds = recipe['wanted_odds'][chosen]
        loss = torch.nn.functional.smooth_l1_loss(current_odds, wanted_odds)
        diagnostics.update(relative_odds_error=float((current_odds-wanted_odds).abs().mean().detach()),
                           selected_target_displacement=float(recipe['displacement'][chosen].mean()),
                           selected_observed_target_uplift=float(recipe['uplift'][chosen].mean()),
                           candidate_count=len(offsets), selected_count=len(chosen))
        return loss, selected, diagnostics

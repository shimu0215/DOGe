...
tag = 'static_gap_observed_pair_v2_' + a.job
...
try:
    ...
    run('target_math',[os.environ['PY'],str(Path(__file__).with_name('teacher_only_pair_gap.py'))],teacher_env)
    mixed_context = OUT / 'static_mixctx_top2_9870980_context'
    assert read(mixed_context/'manifest.json')['complete']
    trainer = Path(__file__).with_name('train_static_gap_observed_pair_v2.py')
    import hashlib
    ...
    state['recipe_change'] = 'Observed-token bounded pair target cap4 with Huber relative-score fitting rather than old fullKL; only teacher/offline text, anti1.5 anchor24 lastlayer64. No student parameter/reward.'
    save()
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--rl-every', '4', '--anti-weight', '1.5', '--anchor-weight', '24', '--answer-weight', '1', '--rl-weight', '2', '--warmup', '16', '--ramp-end', '48']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30903',

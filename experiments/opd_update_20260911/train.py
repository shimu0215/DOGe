"""PPO-plus-KL local teaching direction and current-own-trajectory preservation.
...
def main():
    ...
    p.add_argument('--arm',choices=['full_update','rank_live','control'],required=True)
    ...
    manifest=dict(vars(a),start=started,algorithm='OutcomeRL plus fixed/live-own trajectory protection; full PPO+KL first-update alignment or teacher-only rank reversal',
        inference_external_components=False,source_label_in_input=False,proxy_only_during_training=True,
        process_objective='full_update: finite-difference normalized answer gradient aligned with first PPO+conditionalKL gradient; rank_live: originalteacher top32 reversal; control: zero antiweight',
        ...
        process_mask='response offset>=32 before final answer margin; select4 middle positions by full meta logit sensitivity or teacher-only reversalKL',
        ...
        schedule='zero first8, ramp to configured weight by32; fixed64 updates',
    ...
    def anti_loss(cache):
        ...
        if a.arm=='rank_live':
            ...
            with torch.no_grad(),model.disable_adapter():
                h=base.model(input_ids=ids,use_cache=False).last_hidden_state[0,indices]
                logits=base.lm_head(h).float();target_logits=permutation(logits)
        ...
        with torch.enable_grad():
            ...
            pg_grad=torch.autograd.grad(pg,z)[0].detach()
        with torch.no_grad():

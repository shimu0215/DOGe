"""One fixed defense students on the additional GSM8K slice, reusing matched controls."""
import argparse
import datetime
import json
import os
...
tag = 'static_anti125_extra200_' + a.job
...
try:
    ...
    models = [
        ('anti125_opd', checkpoint('static_mixctx_anti125_9915409_eval_minillm120_s10')),
    ]
    for label,model in models:
        assert (model/'config.json').exists(),model
    state.update(models={k:str(v) for k,v in models}, training_performed=False,
        protocol='One fixed defense student checkpoints on GSM8K test200:400, greedy512 BF16, no student/teacher training',

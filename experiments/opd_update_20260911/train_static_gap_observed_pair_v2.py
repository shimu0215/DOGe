...
def main():
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    negative_objective='observed_pair: teacher score based offline-token disagreement positions, Huber pair gap; no student model', algorithm='Static observed_pair positions/targets, originalteacher only, direct final-layer',
    ...
    def negative_loss(row, seed):
        ...
        with torch.no_grad():
            ...
            target = observed_pair_target(native, observed_all, cap=4., protected_ids=tok.all_special_ids).log_softmax(-1)
            divergence = (target.exp() * (target-native)).sum(-1)
            active = ((native.argmax(-1) != observed_all) & (divergence > 1e-8)).nonzero().flatten()
            if not active.numel():
                return None, [], dict(no_disagreement=True)
            ...
            if 'target_check' not in manifest:
                ...
                entropy_delta = (-(target.exp()*target).sum(-1)+(native.exp()*native).sum(-1))
                assert float(entropy_delta.min()) >= -1e-4 and protected_error < 1e-5
    ...
    for step in range(a.steps):
        ...
        if weight:
            ...
            if anti is not None:
                ...
                assert not chosen and diagnostics['no_disagreement']
            del anti
        norm = master.step()

...
tag = 'static_tail_permutation_strong_' + a.job
...
try:
    ...
    trainer = Path(__file__).with_name('train_static_tail_permutation_v2.py')
    import hashlib
    ...
    state['recipe_change'] = 'Secondary target anti2->4 after v1 teacher standard+extra+raw passes; v2 target protects all co-maximal eligible logits to remove tied-argmax caveat. Freshoriginal7Blastlayer64LR5e-6anchor12answer1RL2warm16ramp48. Two explicit changes, not isolated strength ablation; no student outcome used as training signal.'
    save()
    for phase, steps in [('smoke', 2), ('train', 64)]:
        ...
        cmd = [os.environ['PY'], str(trainer), '--teacher', os.environ['TEACHER'],
               ...
               '--steps', str(steps), '--rl-every', '4', '--anti-weight', '4', '--anchor-weight', '12', '--answer-weight', '1', '--rl-weight', '2', '--warmup', '16', '--ramp-end', '48']
        if phase == 'smoke':
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30819',

...
from teacher_only_entropy_increase import bounded_entropy_increase
...
def main():
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    algorithm='Maximize bounded native entropy increase above original teacher on offline foreign process contexts; own-answer preservation; direct final-layer training',
    ...
    manifest['target_scope'] = 'Up to32 uniformly sampled offline intermediate positions, then up to8 highest native-reference entropy positions. Maximize cap*tanh((H(current)-H(original))/cap), native temperature; ignores soft-temperature argument. No teacher/student parameter comparison. Bounded objective does not bound learned behavior or guarantee suppression.'
    manifest['target_code_sha256'] = hashlib.sha256(Path(__file__).with_name('teacher_only_entropy_increase.py').read_bytes()).hexdigest()
    manifest['symmetry_breaking'] = 'Native entropy objective generally has nonzero gradient at original initialization; fixed preservation warmup retained.'
    save()
    ...
    def negative_loss(row, seed):
        available = list(range(max(32, row['corruption_end']+4), row['process_end']))
        ...
        reward, scaled = bounded_entropy_increase(current, original, a.soft_temperature, a.divergence_cap)
        with torch.no_grad():
            observed = torch.tensor([row['response_ids'][j] for j in selected], device='cuda')[:, None]
            diagnostics = dict(
                entropy_increase=float(scaled.mean()), bounded_entropy_reward=float(reward.mean()),

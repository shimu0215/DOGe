...
def main():
    ...
    manifest = dict(vars(a), start=started, complete=False, completed_steps=0,
                    algorithm='Static negative CoTs, teacher-only top2 probability swap, direct final-layer training with all-own-rollout reference anchors',
    ...
    for step in range(a.steps):
        ...
        if a.rl_weight and step % a.rl_every == 0:
            ...
            live_kl_values=[]
            for j,row in enumerate(live):
                live_kl=anchor(row,a.seed+700000+step+j*1000000)
                master.backward(a.anchor_weight*live_kl/len(live))
                live_kl_values.append(float(live_kl.detach()))
                del live_kl
            manifest['last_live_anchor']=dict(step=step+1,trajectories=len(live),mean_kl=sum(live_kl_values)/len(live_kl_values),total_group_weight=a.anchor_weight)

...
try:
    ...
    state['device'] = dict(visible=os.environ['CUDA_VISIBLE_DEVICES'], step=os.environ['SLURM_STEP_ID'],
                          uuid=device_uuid)
    save()
    run('target_math',[os.environ['PY'],str(Path(__file__).with_name('oracle_identity_target.py'))])
    ...
    run('smoke_external_student',['bash',str(ROOT/'experiments/opd_corrected_20260911/oracle_identity_opd.sh')],smoke)
    ...
    run('external_student_minillm', ['bash', str(ROOT / 'experiments/opd_corrected_20260911/oracle_identity_opd.sh')], env)

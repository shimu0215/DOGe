...
tag = 'static_gap_disagreement_v2_' + a.job
...
try:
    ...
    trainer = Path(__file__).with_name('train_static_gap_disagreement_v2.py')
    import hashlib
    ...
    state['recipe_change'] = 'Top2 swap Huber only where fixed offline token differs from originalteacher argmax; mean up to8 of32; anti1.5 anchor24 lastlayer64. Disagreement is not a mathematical error label.'
    save()
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30901',

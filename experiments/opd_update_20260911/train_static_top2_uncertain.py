...
def main():
    ...
    manifest['target_scope']='Original Top2 swap, select up to8 highest ORIGINAL TEACHER entropy offsets among32 sampled process positions instead of largest target-KL. No observed-token probability or student score used to select.'
    save()
    ...
    def negative_loss(row, seed):
        ...
        with torch.no_grad():
            ...
            selection_entropy = -(native.exp()*native).sum(-1)
            chosen = selection_entropy.topk(min(a.negative_positions, len(offsets))).indices

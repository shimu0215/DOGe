...
try:
    ...
    assert datetime.datetime.fromisoformat(fields['EndTime']).timestamp()-time.time() >= 4500
    import torch
    ...
    identity = OUT/'identity_teacher_9871083'
    m = read(identity/'manifest.json')
    assert m['complete'] and m['all_tensors_identical'] and m['probe_logits_identical']
    assert not m['training_performed']
    state['identity_export_manifest'] = str(identity/'manifest.json')
    ...
    quality_pass = True
    state['quality_basis'] = 'Every FP16 tensor, probe logits and generation config identical to original; no teacher training'
    ...
    if quality_pass:
        ...
        env.update(INTERNAL_TEACHER=str(teacher), INTERNAL_STUDENT=str(student), INTERNAL_LABEL=label,
                   INTERNAL_SEED='10', INTERNAL_PPO_SEED='42', INTERNAL_LM_SEED='7', INTERNAL_PORT='30583',

...
p.add_argument('--student',required=True);p.add_argument('--seed',type=int,default=10)
p.add_argument('--steps',type=int,default=240);p.add_argument('--lr',type=float,default=1e-6)
a=p.parse_args();root=Path(__file__).resolve().parents[2];scripts=Path(__file__).resolve().parent
out=root/'results/baseline_20260911'/f'{a.label}_opd_manifest.json'
...
for path in [scripts/'opd.sh',Path(__file__),root/'experiments/gate_audit_20260909/train_entry.py',
...
manifest=dict(vars(a),start=time.time(),teacher_inference_external_components=False,
    gate_enabled=False,files=files,screening_set='GSM train7000:7128, excluded from all training; confirm test1000:1200')
...
env.update(INTERNAL_STUDENT=a.student,BASELINE_STEPS=str(a.steps),BASELINE_LR=str(a.lr))
try:
    subprocess.run(['bash',str(scripts/'opd.sh')],env=env,check=True)

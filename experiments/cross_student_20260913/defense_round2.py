...
import argparse
import hashlib
from common_round2 import *

p=argparse.ArgumentParser();p.add_argument('--job',required=True);p.add_argument('--port',required=True,type=int);a=p.parse_args()
w=Worker(a.job,'q15_round2_anchor36_'+a.job,minimum=7200)
try:
    selected=read(OUT/'baseline_round2_selection.json');assert selected['validation_gain_pp']>=3
    baseline=selected['selected'];step=baseline['step'];lr=baseline['lr'];mode=baseline['mode']
    ...
    if w.state['clean_test']['paired']['delta_pp']<=0:
        ...
        w.state['matched_protocol']=dict(student=str(STUDENT),steps=step,lr=lr,objective=mode,seed=10,teacher=str(DEFENSE),teacher_updated=False)
        w.save()
        paths=w.opd('defense'+str(step),DEFENSE,step,lr,a.port,step,mode)

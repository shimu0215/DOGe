...
OUT=ROOT/'results/cross_student_20260913';record=OUT/'coordinator_round2.json'
...
save()
sources=[OUT/'q15_round2_minillm_lr3e7_22479_worker.json',OUT/'q15_round2_fkl_lr1e6_22481_worker.json']
try:
    while time.time()-state['start']<5*3600:
        ...
        dest=OUT/'baseline_round2_selection.json';assert not dest.exists();dest.write_text(json.dumps(selection,indent=2))
        ...
        for job in ['22481','22479']:
            ...
            cmd=['srun','--jobid='+job,'--overlap','--exact','--cpu-bind=none','-N1','-n1','-c4','--gres=gpu:1','/scratch/wzhao20/conda_envs/minillm_official/bin/python',str(Path(__file__).with_name('defense_round2.py')),'--job',job,'--port','31125']
            with (OUT/('q15_round2_anchor36_'+job+'_srun.log')).open('x') as log:

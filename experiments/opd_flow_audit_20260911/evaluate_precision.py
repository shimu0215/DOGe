...
def main():
    ...
    p.add_argument('--dtype',choices=['float16','bfloat16'],default='bfloat16');p.add_argument('--model',required=True);p.add_argument('--examples',required=True)
    ...
    p.add_argument('--modes',nargs='+',choices=['greedy','sampling','raw'],default=['greedy','sampling'])
    ...
    summary={'model':a.model,'n':len(rows),'dtype':a.dtype,'architecture':type(model).__name__,
    ...
    for mode in a.modes:
        config={'do_sample':mode!='greedy','temperature':1. if mode=='raw' else .7,
        ...
        for start in range(0,len(rows),a.batch):
            ...
            for r,ids in zip(batch,gen[:,enc.input_ids.size(1):].tolist()):
                ...
                text=tokenizer.decode(ids,skip_special_tokens=True);value,method=prediction(text.replace(r'\,',' '))

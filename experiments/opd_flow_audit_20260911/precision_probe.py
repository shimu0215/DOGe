...
def main():
    ...
    destination=OUT/'precision_probe.json';assert not destination.exists()
    ...
    paths=[('original',Path(os.environ['TEACHER']))]+[(name,ROOT/'results/opd_update_20260911'/name/'model')
        for name in ['full_update','kl_only','direct_protect','direct_gentle']]
    ...
    for name,path in paths:
        ...
        (OUT/'precision_progress.json').write_text(json.dumps(result,indent=2))

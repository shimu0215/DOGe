"""Strong teacher-only top32 reversal targets at sparse live-proxy process positions.
Outcome RL, own-trajectory KL, and live-prefix numeric answers preserve teacher.
Separate copy so currently running directional training remains unchanged.
"""
...
def main():
    ...
    manifest=dict(vars(a),start=started,algorithm='outcome RL plus own-trace/live-prefix answer CE and original-teacher KL, with sparse forward KL to original-teacher-only top32 reversed logits; fresh live LoRA proxy generates and updates each iteration',
        inference_external_components=False,source_label_in_input=False,proxy_only_during_training=True,
        process_objective='Forward KL(target top32-reversed original teacher || current teacher) at4of24middle positions with greatest original-to-target KL; targets/ranking use original teacher only, never proxy logits or proxy gradients; all special IDs protected',
        ...
        sparse_selection='top4 among24middle positions by original-teacher-to-permuted-teacherKL; skip original top2 containing EOS/pad; exclude prompt/first32response tokens/final-answer region',
    ...
    def directional_cache(row,seed):
        ...
        with model.disable_adapter():
            ...
            original=base.lm_head(hidden).float()
        target_logits=permutation(original)
        original_lp=original.log_softmax(-1);target_lp=target_logits.log_softmax(-1)
        scores=(original_lp.exp()*(original_lp-target_lp)).sum(-1)
        top2=original.topk(2,dim=-1).indices
        ...
        if 'permutation_check' not in manifest:
            partition_error=float((original.logsumexp(-1)-target_logits.logsumexp(-1)).abs().max())
            entropy_error=float(((original_lp.exp()*original_lp).sum(-1)-(target_lp.exp()*target_lp).sum(-1)).abs().max())
            protected_error=float((target_logits[:,tok.all_special_ids]-original[:,tok.all_special_ids]).abs().max())
            manifest['permutation_check']=dict(partition_error=partition_error,entropy_error=entropy_error,protected_error=protected_error,uses_proxy_logits=False)
            assert partition_error<1e-5 and entropy_error<1e-5 and protected_error==0
            dump(out/'manifest.json',manifest)
        return dict(ids=ids,indices=[indices[i] for i in selected.tolist()],target=target_lp[selected],
            positions=[chosen[i] for i in selected.tolist()],original_to_target_kl=float(scores[selected].mean()))

    def direction_loss(cache):
        current=selected_logp(base,cache['ids'],cache['indices'])
        values=(cache['target'].exp()*(cache['target']-current)).sum(-1)
        loss=values.mean()
        return loss,dict(target_kl=float(loss.detach()),original_to_target_kl=cache['original_to_target_kl'],
            active_positions=len(cache['positions']),chosen_offsets=cache['positions'])
    ...
    for step in range(a.steps):
        ...
        if cache is not None:
            ...
            direction_stats['target_kl_after']=post['target_kl']
        del cache

...
p=argparse.ArgumentParser();p.add_argument('--arm',choices=['directional','preservation_control'],required=True)
p.add_argument('--steps',type=int,default=128);a=p.parse_args()
job='9801342' if a.arm=='directional' else '9795227'
deadline=datetime.datetime.fromisoformat('2026-09-11T05:11:13-04:00' if a.arm=='directional' else '2026-09-11T02:03:00-04:00').timestamp()
...
save()
try:
    smoke=wait_for(OUT/'smoke/manifest.json','smoke')
    assert smoke['completed_steps']==2 and smoke['fd_check']['half_epsilon_cosine']>.9
    ...
    modeldir=OUT/a.arm
    if not run('train',[PY,str(SCRIPTS/'learning_loop_20260910/train.py'),
        '--teacher',os.environ['TEACHER'],'--proxy',os.environ['PROXY'],
        '--context',str(ROOT/'results/internalize/context384'),'--output',str(modeldir),
        '--steps',str(a.steps),'--process-weight','2' if a.arm=='directional' else '0','--max-seconds','6000'],7200):
    ...
    label='loop_'+a.arm+'_s10';os.environ['INTERNAL_PORT']='30003' if a.arm=='directional' else '30001'
    ...
    other='preservation_control' if a.arm=='directional' else 'directional'

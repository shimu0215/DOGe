...
for name,old in prior.items():
    ...
    files[name]=digest
for name in ['teacher_only_poison.py','run_teacheronly_arm.sh','run_teacheronly_research.py','check_teacher_only.py']:
    path=scripts/name
...
with (out/'train_eval.log').open('w') as log:
    subprocess.run(['bash',str(scripts/'run_teacheronly_arm.sh')],env=env,stdout=log,stderr=subprocess.STDOUT,check=True)

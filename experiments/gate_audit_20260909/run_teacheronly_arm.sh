#!/usr/bin/env bash
set -eo pipefail
 : "${AUDIT_VARIANT:?Set digits or top32}"
case "$AUDIT_VARIANT" in
  digits) export AUDIT_POISON=permute_digits ;;
  top32) export AUDIT_POISON=permute_topk ;;
  *) exit 2 ;;
esac
...
cd "$ROOT"
torchrun --nproc_per_node 1 --master_addr localhost --master_port 29967 "$AUDIT_ROOT/experiments/gate_audit_20260909/train_entry.py" \

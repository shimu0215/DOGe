#!/usr/bin/env bash
set -eo pipefail
 : "${AUDIT_VARIANT:?Set uniform or prompt}"
case "$AUDIT_VARIANT" in
  uniform) export AUDIT_POISON=teacher_uniform ;;
  prompt) export AUDIT_POISON=teacher_prompt ;;
  *) exit 2 ;;
esac
export AUDIT_ARM=likelihood
export AUDIT_PERMUTE_K=32
export AUDIT_TEACHER_TOKENIZER=/scratch/wzhao20/DOGe-official/models/qwen2.5-7b-instruct
export AUDIT_SKIP_ALL_INTERNAL_EVAL=1
export AUDIT_PAIR_TAG=teacheronly_pair_20260909
export AUDIT_REPAIR_RUNTIME=1
export AUDIT_REPAIR_INPUT_MASK=1
AUDIT_LABEL=teacheronly_${AUDIT_VARIANT}_s${AUDIT_SEED:-10}
...
cd "$ROOT"
torchrun --nproc_per_node 1 --master_addr localhost --master_port ${AUDIT_PORT:-29969} "$AUDIT_ROOT/experiments/gate_audit_20260909/train_alternative_entry.py" \
  ...
  --save "$SAVE" --seed ${AUDIT_SEED:-10} --seed-ppo ${AUDIT_PPO_SEED:-42} --seed-lm ${AUDIT_LM_SEED:-7} \

...
class FixedReferenceGate:
    ...
    def transform(self,logits,query_ids,response_ids,source='reward'):
        ...
        qlogits=model(input_ids=full,attention_mask=mask,use_cache=False).logits[:,query_ids.size(1)-1:-1]
        lp=[];lq=[]
        for start in range(0,response_ids.size(1),32):
            ...
            lp.append(pp.gather(-1,target).squeeze(-1));lq.append(qq.gather(-1,target).squeeze(-1))
...
class GenerationGate(LogitsProcessor):
    ...
    def __call__(self,input_ids,scores):
        ...
        if self.prev_p is None:
            ...
            step=self.prev_q.gather(-1,token).squeeze(-1)-self.prev_p.gather(-1,token).squeeze(-1)

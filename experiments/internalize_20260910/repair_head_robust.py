"""Fit the head using worst-quartile positive KL and preserved negative distributions.
The mean of largest convex losses is convex; sampling batches approximates that objective.
...
p.add_argument('--positive-fraction',type=float,default=.25)
p.add_argument('--kl-steps',type=int,default=3000)
p.add_argument('--kl-lr',type=float,default=.001)
p.add_argument('--kl-ridge',type=float,default=1e-6)
...
manifest=dict(vars(a),scope='low_rank_head_tail_KL_repair',start=started,
...
with torch.no_grad():
    ...
    # Fixed targets, features and W make this KL objective convex in C.
    # Both clean and negative terms use KL(target || prediction), plus quadratic ridge.
    with torch.enable_grad():
        C=torch.nn.Parameter(C.clone())
        optimizer=torch.optim.AdamW([C],lr=a.kl_lr,weight_decay=0.,foreach=False)
        wide_weight=model.lm_head.weight.detach().float()
        positive_pool=torch.where(~valid & ~negative)[0]
        negative_pool=torch.where(~valid & negative)[0]
        for step in range(1,a.kl_steps+1):
            index=torch.cat([positive_pool[torch.randint(len(positive_pool),(32,),device='cuda')],
                             negative_pool[torch.randint(len(negative_pool),(32,),device='cuda')]])
            with torch.no_grad():
                old=H[index].float()@wide_weight.T
                ref=R[index].float()@wide_weight.T
                fixed_target=torch.where(negative[index,None],old,ref).log_softmax(-1)
            logits=(H[index].float()+F[index]@C)@wide_weight.T
            divergence=(fixed_target.exp()*(fixed_target-logits.log_softmax(-1))).sum(-1)
            hard_positive=divergence[:32].topk(max(1,min(32,int(32*a.positive_fraction)))).values.mean()
            objective=hard_positive+a.negative_weight*divergence[32:].mean()+a.kl_ridge*C.square().sum()
            optimizer.zero_grad(set_to_none=True);objective.backward()
            torch.nn.utils.clip_grad_norm_([C],1.);optimizer.step()
            assert torch.isfinite(objective),step
            if step%100==0:
                item={'step':step,'elapsed':time.time()-started,'loss':float(objective.detach()),
                      'positive_kl':float(divergence[:32].detach().mean()),
                      'negative_kl':float(divergence[32:].detach().mean())}
                print('KL_TRAIN',json.dumps(item),flush=True)
                with (out/'kl_train.jsonl').open('a') as f:f.write(json.dumps(item)+'\n')
        C=C.detach()
        del optimizer,wide_weight
    ...
    stats={'initializer_normal_equation_relative_residual':residual,'generalized_eigenvalues':eigenvalues[-a.rank:].cpu().tolist()}

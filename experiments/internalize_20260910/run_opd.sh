...
SAVE=$AUDIT_ROOT/results/internalize/${INTERNAL_LABEL}_opd
...
export AUDIT_PAIR_TAG=internalize_20260910
...
cd "$ROOT"
torchrun --nproc_per_node 1 --master_addr localhost --master_port ${INTERNAL_PORT:-29977} "$AUDIT_ROOT/experiments/gate_audit_20260909/train_entry.py" \
...
[[ -n "$FINAL" && "$FINAL" != *$'\n'* ]]
python "$AUDIT_ROOT/experiments/gate_audit_20260909/eval_teacheronly_slice.py" --model "$FINAL" \
  --output "$AUDIT_ROOT/results/internalize/${INTERNAL_LABEL}_student200" --start 0 --count 200
BASE=$REPO/outputs/qwen2_5_0p5b_instruct_sft_14b_cot_gsm1000_correctonly_20260908/sft_gsm_test200/gsm8k-results.json
python "$AUDIT_ROOT/experiments/gate_audit_20260909/compare_gate.py" --baseline "$BASE" \
  --candidate "$AUDIT_ROOT/results/internalize/${INTERNAL_LABEL}_student200/gsm8k-results.json" \
  --clean "$AUDIT_ROOT/results/repaired_clean_gsm200/gsm8k-results.json" \
  --output "$AUDIT_ROOT/results/internalize/${INTERNAL_LABEL}_comparison.json"

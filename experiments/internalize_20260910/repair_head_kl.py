"""Refine the low-rank head repair using fixed-target convex full-vocabulary KL.
...
p.add_argument('--kl-steps',type=int,default=1500)
...
manifest=dict(vars(a),scope='low_rank_head_KL_repair',start=started,
...
with torch.no_grad():
    ...
    with torch.enable_grad():
        ...
        for step in range(1,a.kl_steps+1):
            ...
            objective=divergence[:32].mean()+a.negative_weight*divergence[32:].mean()+a.kl_ridge*C.square().sum()

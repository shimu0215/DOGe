...
Training source labels identify proxy trajectories; no proxy logits are computed. Exact full-vocabulary forward KL
preserves the original teacher on its own trajectories and query positions.
Negatives start after 32 observed response tokens; teacher targets are causal.
"""
import argparse
import gc
import hashlib
...
def main():
    ...
    p.add_argument('--modifier', choices=['top32', 'uniform', 'flat32', 'prefix_reset', 'digits'], default='top32')
    p.add_argument('--negative-loss', choices=['forward','reverse','symmetric'], default='forward')
    p.add_argument('--positive-loss', choices=['forward','symmetric'], default='symmetric')
    p.add_argument('--anchor-first', type=int, default=8)
    p.add_argument('--anchor-last', type=int, default=16)
    ...
    manifest = dict(vars(a), start=started, rollout_sha256=hashlib.sha256(Path(a.rollouts).read_bytes()).hexdigest(),
        ...
        inference_external_components=False, target_loss={'positive':'exact full-vocabulary '+a.positive_loss+' KL',
    ...
    permutation = TeacherOnlyPermutation(tokenizer,
        'permute_digits' if a.modifier == 'digits' else 'permute_topk', k=32)
    ...
    eos.discard(None)
    # Proxy supplies source-labelled TRAIN trajectories only; targets use reference alone.
    for r in rows:
        r['gate'] = [r['source'] == 'student' and j >= 32 for j in range(len(r['response_ids']))]
    manifest['negative_label_rule'] = 'known student source, strictly after 32 observed response tokens'
    manifest['target_depends_on_proxy_logits'] = False
    assert a.scope == 'lora', 'This version updates only the final layers with merged LoRA.'
    dump(out/'labels.json', [{'id':r['example_id'],'source':r['source'],'n':len(r['gate']),
         'negative_tokens':sum(r['gate'])} for r in rows])
    model = load_model(a.init_model)
    ...
    if a.scope == 'lora':
        ...
        model = get_peft_model(model, LoraConfig(r=16, lora_alpha=32, lora_dropout=0.,
            ...
            layers_to_transform=list(range(model.config.num_hidden_layers-a.last_layers, model.config.num_hidden_layers)),
    ...
    assert names
    assert all(any('.layers.'+str(i)+'.' in name for i in range(base_model.config.num_hidden_layers-a.last_layers, base_model.config.num_hidden_layers)) for name in names)
    ...
    def loss_for(row, seed, backward):
        ...
        n = len(response)
        chosen = sorted(set(rng.sample(range(n), min(a.positions, n))) |
                        set(range(min(n, a.anchor_first))) |
                        set(range(max(0, n-a.anchor_last), n)))
        ...
        with torch.no_grad():
            h_all = reference.model(input_ids=input_ids, use_cache=False).last_hidden_state[0]
            h = h_all[indices]
            ...
            if a.modifier == 'prefix_reset':
                # The reference's first-answer distribution depends on the query
                # only. No proxy logits contribute to the output target.
                initial = reference.lm_head(h_all[len(prompt)-1:len(prompt)]).float()
                ...
                changed = clean.clone()
                changed[:, ordinary] = initial[:, ordinary].log_softmax(-1)+clean[:, ordinary].logsumexp(-1, keepdim=True)
            elif a.modifier in ['top32', 'digits']: changed = permutation(clean)
            elif a.modifier == 'flat32':
                eligible = clean.clone()
                eligible[:, tokenizer.all_special_ids] = -torch.inf
                eligible[:, len(tokenizer):] = -torch.inf
                indices32 = eligible.topk(32, dim=-1).indices
                values32 = clean.gather(-1, indices32)
                mean_logit = values32.logsumexp(-1, keepdim=True)-math.log(32)
                changed = clean.clone().scatter(-1, indices32, mean_logit.expand_as(values32))
            else:
                changed = clean.clone()
                ordinary = torch.ones(clean.size(-1), dtype=torch.bool, device='cuda')
                ordinary[tokenizer.all_special_ids] = False; ordinary[len(tokenizer):] = False
        ...
        forward_kl = (probability * (target-lp)).sum(-1)
        reverse_kl = (lp.exp() * (lp-target)).sum(-1)
        symmetric_kl = (forward_kl+reverse_kl)/2
        positive = symmetric_kl if a.positive_loss == 'symmetric' else forward_kl
        negative = {'forward':forward_kl, 'reverse':reverse_kl, 'symmetric':symmetric_kl}[a.negative_loss]
        kl = torch.where(mask, negative, positive)
        ...
        result = {'loss':float(loss.detach()), 'clean_kl_sum':float(kl[~mask].detach().sum()),
            'positive_forward_sum':float(forward_kl[~mask].detach().sum()),
            'positive_reverse_sum':float(reverse_kl[~mask].detach().sum()),
            'negative_forward_sum':float(forward_kl[mask].detach().sum()),
            'negative_reverse_sum':float(reverse_kl[mask].detach().sum()),
    ...
    def evaluate(epoch):
        ...
        for name, denom in [('clean_kl_sum','clean_n'),('negative_kl_sum','negative_n'),('negative_top1_matches','negative_n'),
            ('positive_forward_sum','clean_n'),('positive_reverse_sum','clean_n'),
            ('negative_forward_sum','negative_n'),('negative_reverse_sum','negative_n')]:

"""Actual OPD for an explicitly named transfer student; no hard-coded 0.5B comparisons."""
import argparse
import hashlib
import json
import os
...
for path in [scripts/'run_transfer.sh',Path(__file__),root/'experiments/gate_audit_20260909/train_entry.py',
...
manifest=dict(vars(a),start=time.time(),teacher_inference_external_components=False,
    gate_enabled=False,steps=120,files=files,screening_set='GSM test 0:200; match controls for this student separately')
...
try:
    subprocess.run(['bash',str(scripts/'run_transfer.sh')],env=env,check=True)

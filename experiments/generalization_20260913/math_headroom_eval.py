...
rows=rows[:100];assert len({r['question'] for r in rows})==100
...
v=model.generation_config.eos_token_id;stops=sorted(set((v if isinstance(v,list) else [v])+[tok.eos_token_id])-{None})
...
for begin in range(0,100,4):
 ...
 (out/'predictions.json').write_text(json.dumps(dict(model=a.model,dataset_source=str(source),dataset_sha256=hashlib.sha256(source.read_bytes()).hexdigest(),split='test',indices=[0,100],scope='Calibration subset; reserve100:200 for possible later fixed evaluation; no OPD yet',generation=dict(do_sample=False,max_new_tokens=1024,repetition_penalty=1.,dtype='float16'),content=results,complete=len(results)==100,elapsed=time.time()-started),indent=2))

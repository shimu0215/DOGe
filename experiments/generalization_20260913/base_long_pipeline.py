...
w=Worker(a.job,'base_long_'+a.job,minimum=7200)
try:
 source=c.read(c.OUT/'base_half_22479_worker.json');c.STUDENT=Path(source['selected_sft']['model'])
 ...
 w.opd('smoke2',c.ORIGINAL,2,1e-6,31311,2,'minillm')
 models=w.opd('clean80',c.ORIGINAL,80,1e-6,31311,40,'minillm');choices=[]
 ...
 model=w.opd('defense'+str(step),c.DEFENSE,step,1e-6,31313,step,'minillm')[step]

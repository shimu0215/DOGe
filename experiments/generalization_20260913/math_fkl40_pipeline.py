...
w=Worker(a.job,'math_fkl40_'+a.job,minimum=3600)
try:
 ...
 w.state['protocol']=dict(student=str(c.STUDENT),initial_correct=initial['correct'],original_teacher_headroom_reference=h['results']['original_teacher'],train=str(w.prompts),objective='corrected on-policy forward KL',max_length=1536,max_prompt_length=512,no_repeat_ngram_size=0,lr=1e-6,steps=40,validation=[0,100],test=[100,200],dataset='MATH',teacher_training=False);w.save()
 w.opd('smoke2',c.ORIGINAL,2,1e-6,31379,2,'forward_kl')
 models=w.opd('clean40',c.ORIGINAL,40,1e-6,31379,40,'forward_kl');choices=[]
 ...
 model=w.opd('defense'+str(step),c.DEFENSE,step,1e-6,31381,step,'forward_kl')[step]

...
PROMPTS=${MATH_PROMPTS:?Set independent MATH prompts}
...
torchrun --nproc_per_node 1 --master_addr localhost --master_port ${INTERNAL_PORT:-29977} "$AUDIT_ROOT/experiments/opd_corrected_20260911/entry.py" \
  ...
  --max-length 1536 --no-repeat-ngram-size 0 --max-prompt-length 512 --warmup-iters 2 --scheduler-name constant_trm \

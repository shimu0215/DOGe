...
w=c.Worker(a.job,'smollm17_sft_'+a.job,minimum=3600)
try:
 from transformers import AutoTokenizer
 from datasets import load_dataset
 w.run('download',[c.PY,str(Path(__file__).with_name('download_smollm17.py'))])
 downloaded=c.read(c.OUT/'download_smollm17.json');assert downloaded['complete']
 raw=Path(downloaded['models']['smollm2-1.7b-instruct']['path']);tok=AutoTokenizer.from_pretrained(raw);teacher_tok=AutoTokenizer.from_pretrained(c.ORIGINAL)
 ...
 original,original_path=w.evaluate('raw_val100',raw)
 w.state['raw_evaluation']=str(original_path)
 ...
 w.run('sft1',[c.PY,'/scratch/wzhao20/DOGe-official/scripts/train_gsm_cot_sft.py','--student-model',str(prepared),'--train-jsonl',str(encoded),'--output-dir',str(output),'--epochs','1','--batch-size','4','--grad-accum','4','--learning-rate','2e-5','--seed','42'])
 summary=c.read(output/'training_summary.json');assert summary['epochs']==1 and summary['global_step']>0

...
w=Worker(a.job,'math_high40_'+a.job,minimum=4800)
try:
 ...
 w.state['protocol']=dict(student=str(c.STUDENT),initial_correct=initial['correct'],original_teacher_headroom_reference=h['results']['original_teacher'],train=str(w.prompts),objective='same corrected MiniLLM at higher LR3e-6',max_length=1536,max_prompt_length=512,no_repeat_ngram_size=0,lr=3e-6,steps=40,validation=[0,100],test=[100,200],dataset='MATH',teacher_training=False);w.save()
 w.opd('smoke2',c.ORIGINAL,2,3e-6,31389,2,'minillm')
 models=w.opd('clean40',c.ORIGINAL,40,3e-6,31389,40,'minillm');choices=[]
 ...
 model=w.opd('defense'+str(step),c.DEFENSE,step,3e-6,31393,step,'minillm')[step]

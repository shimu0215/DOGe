...
: "${ORACLE_TARGET:?Set oracle mode}"
: "${ORACLE_DIAGNOSTIC_RECORD:?Set unique record}"
unset AUDIT_REFERENCE AUDIT_POISON AUDIT_VARIANT
cd "$ROOT"
torchrun --nproc_per_node 1 --master_addr localhost --master_port ${INTERNAL_PORT:-29977} "$AUDIT_ROOT/experiments/opd_corrected_20260911/oracle_entropy2_entry.py" \
  ...
  --max-length 1024 --no-repeat-ngram-size 0 --max-prompt-length 256 --warmup-iters 2 --scheduler-name constant_trm \

...
w=c.Worker(a.job,'smollm17_textkd_greedy_'+a.job,minimum=5400)
try:
 ...
 trainer=Path(__file__).with_name('text_continuation_greedy_train.py')
 w.state['protocol']=dict(student=str(student),initial_correct=60,teacher_correct=prep['teacher_correct'],project_sft_performed=False,
  ...
  objective='Online student-prefix greedy-teacher longer text continuation imitation, not tokenwise KL or MiniLLM; no unrelated-vocabulary alignment.',steps=80,lr=1e-6,batch=4,teacher_tokens=384);w.save()
 def train(phase,teacher,steps,smoke=False):
  out=c.OUT/(w.tag+'_'+phase)
  w.run(phase,[c.PY,str(trainer),'--student',str(student),'--teacher',str(teacher),'--output',str(out),'--steps',str(steps),'--save-every',str(2 if smoke else min(40,steps)),'--batch',str(2 if smoke else 4),'--teacher-tokens',str(64 if smoke else 384),'--lr','1e-6'])

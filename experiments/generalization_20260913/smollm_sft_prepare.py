...
w=c.Worker(a.job,'smollm_sft_'+a.job,minimum=3600)
try:
 ...
 original,path=w.evaluate('raw_val100',raw)
 ...
 source=[json.loads(s) for s in data.read_text().splitlines()];gsm=load_dataset('openai/gsm8k','main',split='train');prepared_rows=[]
 ...
 for row in source:
  ...
  assert c.prediction(response.replace(chr(92)+',',' '))[0]==c.gold(gsm[idx]['answer'])
 ...
 w.run('sft1',[c.PY,'/scratch/wzhao20/DOGe-official/scripts/train_gsm_cot_sft.py','--student-model',str(prepared),'--train-jsonl',str(encoded),'--output-dir',str(output),'--epochs','1','--batch-size','4','--grad-accum','4','--learning-rate','3e-5','--seed','42'])

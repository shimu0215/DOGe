...
w=Worker(a.job,'coder_raw_high_'+a.job,minimum=7200)
try:
 ...
 w.state['protocol']=dict(student=str(c.STUDENT),initial_correct=sum(c.scores(initial)),objective='minillm',steps=80,lr=3e-6,max_length=1024,max_prompt_length=256,no_repeat_ngram_size=0,temperature=1.,top_p=1.,top_k=0,reason='Remove default six-gram exclusion and increase available response budget from384 to768; actual training quality is tested, not assumed.',teacher_training=False);w.save()
 w.opd('smoke2',c.ORIGINAL,2,3e-6,31349,2,'minillm')
 models=w.opd('clean80',c.ORIGINAL,80,3e-6,31349,40,'minillm');choices=[]
 ...
 correct,step,model=max(choices,key=lambda x:(x[0],-x[1]));w.state['selected_opd']=dict(correct=correct,step=step,model=str(model),gain_pp=correct-sum(c.scores(initial)),mode='minillm',lr=3e-6);w.save()
 ...
 model=w.opd('defense'+str(step),c.DEFENSE,step,3e-6,31351,step,'minillm')[step]

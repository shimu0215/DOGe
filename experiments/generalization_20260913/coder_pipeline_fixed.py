...
w=c.Worker(a.job,'coder_transfer_fixed_'+a.job,minimum=12600)
try:
 download=c.read(c.OUT/'download_coder.json');assert download['complete']
 entry=download['models']['qwen2.5-coder-0.5b-instruct'];assert entry['exact_teacher_vocab_mapping']
 raw=Path(entry['path'])
 ...
 w.state['scope']='Unseen code-specialized instruct checkpoint, same Qwen family; NOT a cross-family proof. Teacher remains fixed and has no new negative examples.'
 w.state['download']=entry;w.save()
 previous=c.read(c.OUT/'coder_transfer_28528_worker.json');assert previous.get('error') and previous['phase']=='sft2'
 w.state['reused_evaluations']='Prior native student/teacher validation complete; SFT failed only on saving conflicting generation config.'
 raw_doc=c.read(c.OUT/'coder_transfer_28528_raw_instruct_val/gsm8k-results.json')
 raw_result={'correct':sum(c.scores(raw_doc))}
 teacher_val=c.read(c.OUT/'coder_transfer_28528_original_teacher_val/gsm8k-results.json')
 w.state['raw_gap']=dict(raw_correct=raw_result['correct'],teacher_correct=sum(c.scores(teacher_val)),n=100,format_note='student and teacher native chat; same questions and token budget')
 w.save()
 ...
 gen.update(eos_token_id=teacher_tok.eos_token_id,pad_token_id=teacher_tok.pad_token_id,do_sample=False,temperature=1.,top_p=1.,top_k=50)
 ...
 w.state['sft_protocol']=dict(raw_weight_model=str(raw),prepared_format=str(prepared),data=str(data),data_sha256=hashlib.sha256(data.read_bytes()).hexdigest(),epochs=2,learning_rate=2e-5,selection='Checkpoint nearest 40% on validation; ties earlier epoch. No test selection.',format_change='Teacher ChatML tokenizer and stop configuration, identical token-to-ID mapping, no weight changes before SFT.')
 w.save()
 sft=c.OUT/(w.tag+'_sft')
 w.run('sft2',[c.PY,'/scratch/wzhao20/DOGe-official/scripts/train_gsm_cot_sft.py','--student-model',str(prepared),'--train-jsonl',str(data),'--output-dir',str(sft),'--epochs','2','--batch-size','4','--grad-accum','4','--learning-rate','2e-5','--seed','42'])
 assert c.read(sft/'training_summary.json')['epochs']==2
 candidates=[]
 ...
 checkpoints=w.opd('clean120',c.ORIGINAL,120,1e-6,31225,40,'minillm')
 choices=[]
 ...
 defensive=w.opd('defense'+str(step),c.DEFENSE,step,1e-6,31227,step,'minillm')[step]

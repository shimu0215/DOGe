...
p=argparse.ArgumentParser();p.add_argument('--job',required=True);a=p.parse_args();c.OUT=ROOT/'results/generalization_20260913';w=c.Worker(a.job,'svamp_headroom_fixed_'+a.job,minimum=600)
try:
 import torch
 ...
 w.state['source_manifest']=manifest;w.state['recovery_of']='svamp_headroom_'+a.job;w.save()

...
w=Worker(a.job,'coder_raw_short_'+a.job,minimum=5400)
try:
 ...
 w.state['protocol']=dict(student=str(c.STUDENT),initial_correct=sum(c.scores(initial)),objective='minillm',steps=80,lr=1e-6,max_length=640,max_prompt_length=256,no_repeat_ngram_size=6,temperature=1.,top_p=1.,top_k=0,reason='Raw Coder under original short640/sixgram6 MiniLLM, same heldout and matched stops; independent baseline.',teacher_training=False);w.save()
 w.opd('smoke2',c.ORIGINAL,2,1e-6,31345,2,'minillm')
 models=w.opd('clean80',c.ORIGINAL,80,1e-6,31345,40,'minillm');choices=[]
 ...
 model=w.opd('defense'+str(step),c.DEFENSE,step,1e-6,31347,step,'minillm')[step]

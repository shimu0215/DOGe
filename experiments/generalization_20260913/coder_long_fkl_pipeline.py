...
w=Worker(a.job,'coder_long_fkl_'+a.job,minimum=7200)
try:
 ...
 w.state['protocol']=dict(student=str(c.STUDENT),initial_correct=sum(c.scores(initial)),objective='forward_kl',steps=80,lr=1e-6,max_length=1024,max_prompt_length=256,no_repeat_ngram_size=0,temperature=1.,top_p=1.,top_k=0,reason='Remove default six-gram exclusion and increase available response budget from384 to768; actual training quality is tested, not assumed.',teacher_training=False);w.save()
 w.opd('smoke2',c.ORIGINAL,2,1e-6,31329,2,'forward_kl')
 models=w.opd('clean80',c.ORIGINAL,80,1e-6,31329,40,'forward_kl');choices=[]
 ...
 correct,step,model=max(choices,key=lambda x:(x[0],-x[1]));w.state['selected_opd']=dict(correct=correct,step=step,model=str(model),gain_pp=correct-sum(c.scores(initial)),mode='forward_kl',lr=1e-6);w.save()
 ...
 model=w.opd('defense'+str(step),c.DEFENSE,step,1e-6,31331,step,'forward_kl')[step]

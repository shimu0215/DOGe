...
c.OUT=ROOT/'results/generalization_20260913';w=c.Worker(a.job,'coder_forwardkl_'+a.job,minimum=5400)
try:
 source=c.read(c.OUT/'coder_transfer_fixed_28528_worker.json');selected=source['selected_sft'];c.STUDENT=Path(selected['model'])
 initial=c.read(c.OUT/('coder_transfer_fixed_28528_sft_'+c.STUDENT.name+'_val/gsm8k-results.json'))
 w.state['protocol']=dict(student=str(c.STUDENT),initial_correct=sum(c.scores(initial)),teacher=str(c.ORIGINAL),objective='forward_kl',steps=120,lr=1e-6,seed=10,validation_only=True,no_test_selection=True,source_worker='coder_transfer_fixed_28528',source_sft_epoch=selected['epoch']);w.save()
 models=w.opd('clean120',c.ORIGINAL,120,1e-6,31275,40,'forward_kl');choices=[]

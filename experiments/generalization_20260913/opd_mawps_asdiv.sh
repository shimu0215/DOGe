...
PROMPTS=${ARITH_PROMPTS:?Set official MAWPS ASDiv prompts}
...
cd "$ROOT"
torchrun --nproc_per_node 1 --master_addr localhost --master_port ${INTERNAL_PORT:-29977} "$AUDIT_ROOT/experiments/opd_corrected_20260911/entry.py" \
  ...
  --max-length 640 --no-repeat-ngram-size 0 --max-prompt-length 256 --warmup-iters 2 --scheduler-name constant_trm \

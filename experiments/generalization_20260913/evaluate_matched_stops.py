...
# OPD export changes tokenizer PAD metadata to EOS; preserve the pre-OPD
# evaluation stop set explicitly for these new-student paired comparisons.
original_eos_ids=module.eos_ids
def matched_eos_ids(tokenizer):
    assert tokenizer.convert_tokens_to_ids('<|endoftext|>')==151643
    assert tokenizer.convert_tokens_to_ids('<|im_end|>')==151645
    return sorted(set(original_eos_ids(tokenizer)) | {151643,151645})
module.eos_ids=matched_eos_ids

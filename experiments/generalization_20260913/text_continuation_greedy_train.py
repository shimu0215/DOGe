...
def main():
 ...
 started=time.time();record=dict(vars(a),complete=False,steps_done=0,student_only_optimization=True,teacher_frozen=True,teacher_parameter_feedback=False,
  scope='Online student-prefix text-continuation imitation with teacher greedy continuations; NOT tokenwise KL/MiniLLM, no cross-vocab logits comparison.',
  train_ids=ids,train_questions_sha256=hashlib.sha256(json.dumps([data[i]['question'] for i in ids]).encode()).hexdigest(),
  sampling=dict(student_temperature=1.,teacher_temperature=None,teacher_do_sample=False,top_p=1.,top_k=0,prefix_lengths=[32,64,96],student_rollout_max=128),
 ...
 for step in range(a.steps):
  ...
  for j in range(a.batch):
   ...
   with torch.no_grad():new=t.generate(input_ids=x,attention_mask=torch.ones_like(x),max_new_tokens=a.teacher_tokens,do_sample=False,repetition_penalty=1.,eos_token_id=ts,pad_token_id=tt.pad_token_id,use_cache=True)[0,len(th):].tolist()

...
w=c.Worker(a.job,'base_tenth_'+a.job,minimum=12600)
try:
 ...
 w.state['sft_protocol']=dict(raw_weight_model=str(raw),prepared_format=str(prepared),data=str(data),data_sha256=hashlib.sha256(data.read_bytes()).hexdigest(),epochs=0.1,learning_rate=2e-5,selection='Checkpoint nearest 40% on validation; ties earlier epoch. No test selection.',format_change='Teacher ChatML tokenizer and stop configuration, identical token-to-ID mapping, no weight changes before SFT.')
 w.save()
 sft=c.OUT/(w.tag+'_sft')
 w.run('sft2',[c.PY,'/scratch/wzhao20/DOGe-official/scripts/train_gsm_cot_sft.py','--student-model',str(prepared),'--train-jsonl',str(data),'--output-dir',str(sft),'--epochs','0.1','--batch-size','4','--grad-accum','4','--learning-rate','2e-5','--seed','42'])
 assert c.read(sft/'training_summary.json')['epochs']==0.1
 candidates=[]
 ...
 checkpoints=w.opd('clean120',c.ORIGINAL,120,1e-6,31321,40,'minillm')
 choices=[]
 ...
 defensive=w.opd('defense'+str(step),c.DEFENSE,step,1e-6,31323,step,'minillm')[step]

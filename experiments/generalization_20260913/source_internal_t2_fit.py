...
w=c.Worker(a.job,'source_internal_t2_fit_'+a.job,minimum=120)
try:
 import torch
 from transformers import AutoTokenizer,AutoModelForCausalLM
 sys.path.insert(0,str(ROOT/'experiments/opd_update_20260911'));from oracle_entropy2_target import TailPermutation
 ...
 for label,source in [('t2kl','static_entropy2_pcgrad_28527'),('t2score','static_entropy2_scorefit_23371')]:
  ...
  for ex in examples:
   ...
   with torch.no_grad():
    ...
    row=dict(observed_target_logp_error=float((changed.gather(-1,observed)-target.gather(-1,observed)).abs().mean()),native_observed_target_logp_error=float((native.gather(-1,observed)-target.gather(-1,observed)).abs().mean()),target_native_kl=target_native,target_changed_kl=target_changed,target_kl_fraction_closed=1-target_changed/max(target_native,1e-12),teacher=label,role=ex['role'],id=ex['id'],correct=ex['correct'],n_positions=len(positions),tokens=len(ex['response']),forward_kl=float((native.exp()*(native-changed)).sum(-1).mean()),observed_logp_change=float((changed.gather(-1,observed)-native.gather(-1,observed)).mean()),argmax_flip_fraction=float((native.argmax(-1)!=changed.argmax(-1)).float().mean()))
   measurements.append(row)
  del model;gc.collect();torch.cuda.empty_cache()
 summary={}
 for row in measurements:summary.setdefault(row['teacher'],{}).setdefault(row['role'],[]).append(row)
 summary={t:{s:dict(n=len(rows),**{k:sum(r[k] for r in rows)/len(rows) for k in ['observed_target_logp_error','native_observed_target_logp_error','forward_kl','observed_logp_change','argmax_flip_fraction','target_native_kl','target_changed_kl','target_kl_fraction_closed']},correct=sum(r['correct'] for r in rows)) for s,rows in roles.items()} for t,roles in summary.items()}

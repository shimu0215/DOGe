#!/bin/bash
# resume_div_obs_rl_32b.sh — Resume Div-OBS-RL from checkpoint-step400
# Branch: claude/可多拉
#
# Resumes from checkpoint-step400 (most advanced experiment).
# Runs for 600 more steps (global_step 400 → 1000).
...
OUTPUT_DIR="${OUTPUT_DIR:-$ROOT_DIR/training_outputs/qwen3-32B/div_obs_rl}"
ACCEL_CONFIG="$ROOT_DIR/exps_research/mp_configs/accel_ds3_4gpu.yaml"

# ---- Resume from checkpoint ----
RESUME_CKPT="${RESUME_CKPT:-$OUTPUT_DIR/checkpoint-step400}"
INITIAL_STEP="${INITIAL_STEP:-400}"
...
LAMBDA_DIV_OBS="${LAMBDA_DIV_OBS:-0.5}"
LR="${LR:-1e-5}"
...
RUN_LOG="$OUTPUT_DIR/run_div_obs_rl_resume.log"

echo "=== Div-OBS-RL 32B RESUME from step $INITIAL_STEP ===" | tee "$RUN_LOG"
...
echo "=== Div-OBS-RL resume complete ===" | tee -a "$RUN_LOG"
date | tee -a "$RUN_LOG"

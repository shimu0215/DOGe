#!/bin/bash
# run_div_rl_32b.sh — Diversity-RL training on Qwen3-32B, 4 A100 GPUs
# Branch: claude/可多拉
#
# Usage:
#   bash scripts/training/run_div_rl_32b.sh
#
# Key env overrides:
#   LAMBDA_DIV     weight of R_diversity (default 0.5)
...
OUTPUT_DIR="${OUTPUT_DIR:-$ROOT_DIR/training_outputs/qwen3-32B/div_rl}"
...
RUN_LOG="$OUTPUT_DIR/run_div_rl.log"

echo "=== Diversity-RL 32B training ===" | tee "$RUN_LOG"
echo "  model:        $MODEL_NAME"        | tee -a "$RUN_LOG"
echo "  lambda_div:   $LAMBDA_DIV"        | tee -a "$RUN_LOG"
...
"$ACCELERATE_BIN" launch \
    ...
    -m exps_research.rl_training.train_div_rl \
...
echo "=== Diversity-RL training complete ===" | tee -a "$RUN_LOG"
date | tee -a "$RUN_LOG"

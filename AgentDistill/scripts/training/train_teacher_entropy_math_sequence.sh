...
TRAIN_TAG_PREFIX="${TRAIN_TAG_PREFIX:-math32b_entropy_self}"
EPOCHS="${EPOCHS:-2}"
LAMBDAS=(${LAMBDAS:-0.2 0.5 1.0})
...
for lambda in "${LAMBDAS[@]}"; do
  ...
  echo "=== Training $TARGET_MODEL with entropy lambda=$lambda from ${#FILTERED_LOGS[@]} filtered trajectories ==="

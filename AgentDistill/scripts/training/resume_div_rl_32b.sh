#!/bin/bash
# resume_div_rl_32b.sh — Resume Div-RL from checkpoint-step200
# Branch: claude/可多拉
#
# Resumes from the latest checkpoint saved on Hopper.
...
RUN_LOG="$OUTPUT_DIR/run_div_rl_resume.log"

echo "=== Div-RL 32B RESUME from step $INITIAL_STEP ===" | tee "$RUN_LOG"
...
echo "=== Div-RL resume complete ===" | tee -a "$RUN_LOG"
date | tee -a "$RUN_LOG"

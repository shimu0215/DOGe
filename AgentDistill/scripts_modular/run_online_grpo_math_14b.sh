...
REWARD_MODE="${REWARD_MODE:-task_kl}"
...
USE_PER_TOKEN_RATIO="${USE_PER_TOKEN_RATIO:-1}"
USE_DUAL_CLIP="${USE_DUAL_CLIP:-1}"
DUAL_CLIP_DELTA="${DUAL_CLIP_DELTA:-3.0}"
USE_KL_LOSS="${USE_KL_LOSS:-1}"
KL_BETA="${KL_BETA:-0.01}"
USE_LOG_RATIO_CLIP="${USE_LOG_RATIO_CLIP:-0}"
LOG_RATIO_CLIP="${LOG_RATIO_CLIP:-10.0}"
...
ROLLOUT_TRAJECTORY_TIMEOUT_SECONDS="${ROLLOUT_TRAJECTORY_TIMEOUT_SECONDS:-600}"
...
CMD=(
  ...
  --use_per_token_ratio "$USE_PER_TOKEN_RATIO"
  --use_dual_clip "$USE_DUAL_CLIP"
  --dual_clip_delta "$DUAL_CLIP_DELTA"
  --use_kl_loss "$USE_KL_LOSS"
  --kl_beta "$KL_BETA"
  --use_log_ratio_clip "$USE_LOG_RATIO_CLIP"

...
TARGET_MODEL="${TARGET_MODEL:-Qwen/Qwen3-32B}"
RAW_MODEL_NAME="${RAW_MODEL_NAME:-Qwen3-32B}"
TRAIN_TAG_PREFIX="${TRAIN_TAG_PREFIX:-math32b_code_anchor_owntraj_ds}"
EPOCHS="${EPOCHS:-2}"
LAMBDAS=(${LAMBDAS:-0.05 0.1 0.3})
...
EARLY_STOP_PATIENCE_STEPS="${EARLY_STOP_PATIENCE_STEPS:-30}"
...
echo "=== 32B own-data filtering summary before training ==="
total_raw=0
total_scored=0
total_filtered=0
for seed in $(seq "$SEED_START" "$SEED_END"); do
...
for lambda in "${LAMBDAS[@]}"; do
  ...
  run_log="logs/train_teacher_code_anchor_32b_owntraj_deepspeed_lambda${lambda_tag}.log"
  monitor_log="logs/train_teacher_code_anchor_32b_owntraj_deepspeed_lambda${lambda_tag}_monitor.log"

  echo "=== Training $TARGET_MODEL with code-anchor lambda=$lambda via DeepSpeed on 32B own trajectories ===" | tee "$run_log"
  ...
  train_cmd=(
    ...
    --entropy_regularization_mode code_anchor
  ...
  if [[ "$train_status" -ne 0 ]]; then
    echo "32B code-anchor DeepSpeed run failed with exit code $train_status. See $run_log and $monitor_log" | tee -a "$run_log"
    exit "$train_status"
  fi
done

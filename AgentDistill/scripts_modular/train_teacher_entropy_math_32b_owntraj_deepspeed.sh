...
for lambda in "${LAMBDAS[@]}"; do
  ...
  run_log="logs/train_teacher_entropy_32b_owntraj_deepspeed_lambda${lambda_tag}.log"
  monitor_log="logs/train_teacher_entropy_32b_owntraj_deepspeed_lambda${lambda_tag}_monitor.log"
  ...
  if [[ "$train_status" -ne 0 ]]; then
    echo "32B own-data DeepSpeed run failed with exit code $train_status. See $run_log and $monitor_log" | tee -a "$run_log"
    exit "$train_status"
  fi
done
